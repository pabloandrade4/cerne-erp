...
function serialize(row) {
  return {
    ...
    dataInicio: row.data_inicio ? String(row.data_inicio).slice(0, 10) : null,
    dataFim: row.data_fim ? String(row.data_fim).slice(0, 10) : null,
    ativo: row.ativo,
...
async function atualizarDespesaFixa(id, body) {
  ...
  const bodyComContexto = {
    ...
    dataInicio: body.dataInicio !== undefined ? body.dataInicio : String(atual.data_inicio).slice(0, 10),
    ...body,
  };
