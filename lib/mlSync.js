...
async function sincronizarConta(contaId) {
  ...
  do {
    const search = await ml.apiGet(
      `/orders/search?seller=${conta.ml_user_id}&sort=date_desc&offset=${offset}&limit=${limit}`,
      accessToken
    );
  ...
  return { total: total || 0, importados, erros };
}
