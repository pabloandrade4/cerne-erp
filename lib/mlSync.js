      ...
      const { rows: updated } = await pool.query(
        `UPDATE ml_contas
         SET access_token_enc = $1, refresh_token_enc = $2, token_expires_at = $3,
             escopo_oauth = COALESCE($4, escopo_oauth),
        ...
        [
          ...
          // Toda renovação de token também devolve `scope` — grava de graça
          // (IA de Promoções, ver lib/mlPermissoes.js). COALESCE porque
          // algumas respostas de refresh não repetem o campo; nesse caso
          // mantém o último valor conhecido em vez de apagar.
          // .slice() de segurança: a coluna já é TEXT (sem limite), mas um
          // valor absurdamente grande nunca deveria travar a renovação do
          // token por causa de um campo puramente informativo.
...
// ============================================================
// Funções PURAS (sem I/O) que fazem os cálculos dos Bugs 1 e 2 da
// reconciliação PF ERP x Mercado Turbo (24/08/2026 — ver
// docs/04-alteracoes.md). Extraídas à parte de importarPedidoInterno só pra
// poderem ser testadas direto (server/test/mlSync.test.js) com JSON real de
// pedidos, sem precisar de banco/API — o comportamento em produção não muda,
// importarPedidoInterno chama exatamente estas mesmas funções.
// ============================================================

// Bug 1: custosEnvio (resposta de /shipments/{id}/costs) é do ENVIO inteiro,
// não do pedido — o rateio entre pedidos que compartilham o mesmo envio
// acontece depois, em importarPedidoInterno (precisa consultar o banco pra
// saber com quantos pedidos dividir). Esta função só extrai o valor BRUTO
// (não rateado) do frete do comprador e do vendedor desta conta.
function extrairFreteDoCustosEnvio(custosEnvio, mlUserId) {
  const freteComprador = num(custosEnvio && custosEnvio.receiver && custosEnvio.receiver.cost);
  const senders = (custosEnvio && custosEnvio.senders) || [];
  const senderDaConta = senders.find((s) => String(s.id) === String(mlUserId));
  const freteVendedor = num((senderDaConta || senders[0] || {}).cost);
  return { freteComprador, freteVendedor };
}

// Bug 1: rateio igual entre quem compartilha o mesmo ml_shipping_id.
// divisor <= 1 (ou não numérico) devolve o valor bruto sem dividir — é o
// caso normal (pedido sozinho no próprio envio, sem "carrinho").
function ratearValor(valorBruto, divisor) {
  if (valorBruto === null) return null;
  const d = typeof divisor === 'number' && divisor > 0 ? divisor : 1;
  return round2(valorBruto / d);
}

// Bug 2 (ver comentário completo mais abaixo, em importarPedidoInterno):
// order_items[].sale_fee vem da API por UNIDADE — a comissão real da linha
// é sale_fee × quantity, igual valor_total_item já faz com unit_price.
function calcularTaxaVendaItem(saleFeeUnitario, quantidade) {
  const saleFee = num(saleFeeUnitario);
  if (saleFee === null) return null;
  const qtd = typeof quantidade === 'number' && quantidade > 0 ? quantidade : 1;
  return round2(saleFee * qtd);
}

function calcularTaxaVendaTotal(itens) {
  if (!itens || !itens.some((it) => typeof it.sale_fee === 'number')) return null;
  return round2(
    itens.reduce((sum, it) => {
      const linha = calcularTaxaVendaItem(it.sale_fee, it.quantity);
      return linha === null ? sum : sum + linha;
    }, 0)
  );
}
...
async function importarPedidoInterno(conta, orderId, accessToken) {
  ...
  if (order.shipping && order.shipping.id) {
    ...
    try {
      ...
      ({ freteComprador, freteVendedor } = extrairFreteDoCustosEnvio(custosEnvio, conta.ml_user_id));
    } catch (e) {
  ...
  // Correção de 24/08/2026 (Bug 2 da reconciliação PF ERP x Mercado Turbo —
  // ver docs/04-alteracoes.md): order_items[].sale_fee vem da API POR
  // UNIDADE, não pela linha inteira — confirmado com pedidos reais (ex:
  // 2000018078185798, quantity=2, sale_fee=5.66 → comissão real da venda é
  // 11,32, não 5,66). Antes desta correção, um pedido com quantidade > 1
  // subestimava a comissão (e por isso superestimava a margem) na mesma
  // proporção da quantidade. Quando algum item não tem quantity numérico,
  // assume 1 (não bloqueia o cálculo por causa de um dado que normalmente
  // vem preenchido).
  ...
  // Correção de 24/08/2026 (Bug 1 da reconciliação PF ERP x Mercado Turbo —
  // ver docs/04-alteracoes.md): quando o comprador finaliza um carrinho com
  // mais de um pedido do mesmo vendedor num só envio (mesmo
  // order.shipping.id — confirmado com pedidos reais, ex: 2000018075073530
  // e 2000018075078724, mesmo pack_id e mesmo ml_shipping_id), a API de
  // custos de frete (/shipments/{id}/costs) devolve o custo do ENVIO
  // INTEIRO, não do pedido individual — sincronizar cada pedido separado
  // fazia cada um "herdar" o frete cheio, dobrando (ou mais) o frete
  // somado. Sempre que este pedido tem um ml_shipping_id, depois de salvar,
  // rateia o frete IGUALMENTE entre todos os pedidos desta conta que
  // compartilham o mesmo ml_shipping_id — inclui os já sincronizados antes
  // (e reajusta o frete deles também, pra nunca ficar com um valor
  // desatualizado quando um novo pedido do mesmo envio aparece depois).
  // Sempre recalcula a partir do valor BRUTO (freteVendedor/freteComprador
  // computados acima, direto da API) — nunca a partir de um valor já
  // rateado salvo antes — pra ressincronizar não ir dividindo o valor
  // repetidamente a cada vez.
  if (order.shipping && order.shipping.id) {
    const { rows: irmaos } = await pool.query(
      `SELECT id FROM ml_pedidos WHERE conta_ml_id = $1 AND ml_shipping_id = $2`,
      [conta.id, order.shipping.id]
    );
    const divisor = irmaos.length || 1;
    const freteVendedorRateado = ratearValor(freteVendedor, divisor);
    const freteCompradorRateado = ratearValor(freteComprador, divisor);
    await pool.query(
      `UPDATE ml_pedidos SET frete_vendedor = $1, frete_comprador = $2, atualizado_em = now()
       WHERE conta_ml_id = $3 AND ml_shipping_id = $4`,
      [freteVendedorRateado, freteCompradorRateado, conta.id, order.shipping.id]
    );
  }
  ...
  for (const it of itens) {
    ...
    // taxa_venda guarda a comissão TOTAL da linha (sale_fee por unidade ×
    // quantidade), igual valor_total_item já faz com o preço — ver
    // explicação do Bug 2 acima, em taxaVendaTotal.
  ...
  for (const pg of order.payments || []) {
    await pool.query(
      `INSERT INTO ml_pedido_pagamentos (
         ...
         transaction_amount, taxes_amount, shipping_cost, marketplace_fee, coupon_amount, installments,
      ...
      [
        ...
        // docs/04-alteracoes.md): payments[].coupon_amount é um desconto
        // (cupom Mercado Livre/PIX) aplicado no pagamento que NÃO aparece em
        // order.total_amount — confirmado com pedidos reais (4 exemplos:
        // R$1,77 / R$1,67 / R$1,77 / R$0,86+R$1,81, batendo exatamente com a
        // diferença de margem observada). Guardado aqui (além de já estar
        // dentro de raw_pagamento) pra lib/relatorioVendas.js poder somar
        // sem reprocessar o JSON bruto a cada consulta.
        num(pg.coupon_amount),
...
module.exports = {
  ...
  // Funções puras exportadas só pra teste automatizado (server/test/) — ver
  // comentário acima delas. importarPedidoInterno (não exportada, precisa de
  // API/banco de verdade) usa exatamente estas mesmas funções em produção.
