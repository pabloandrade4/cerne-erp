...
const { listarRecebimentosMl, listarRecebimentosMlRecebidosNoPeriodo } = require('./recebimentosMl');
...
function somarPorDia(rows, campoData) {
  const mapa = new Map();
  for (const r of rows) {
    const dia = String(r[campoData]).slice(0, 10);
...
async function gerarFluxoDeCaixa({ empresaId, periodoChave, desde: desdeQuery, ate: ateQuery }) {
  ...
  // Recebimentos de marketplace (Mercado Livre — mesma fonte de
  // lib/recebimentosMl.js/tela Recebimentos), separados em dois grupos —
  // NUNCA somados juntos, senão duplica previsto+realizado (pedido
  // explícito do usuário):
  //   PREVISTO: recebimentos ainda 'a_receber'/'disponivel' cuja VENDA caiu
  //     dentro do período — sem data de liberação real (ver
  //     lib/recebimentosMl.js), por isso entram só como um TOTAL do
  //     período (não plotados num dia específico do gráfico — nunca
  //     inventamos a data em que o marketplace vai liberar o dinheiro).
  //   REALIZADO: recebimentos já 'recebido' cuja data EFETIVA de
  //     recebimento caiu dentro do período — mesmo critério de
  //     recebidosNoPeriodo (contas_receber) acima, e por isso somado no
  //     MESMO mapa/série diária (mapaRecebidos), nunca num total à parte.
  const desdeInstant = inicioDoDiaBRTDeString(desde);
  const ateInstant = inicioDoDiaBRTDeString(somarDiasData(ate, 1));
  const [recebimentosMlDoPeriodo, recebimentosMlRecebidos] = await Promise.all([
    listarRecebimentosMl({ empresaId, desde: desdeInstant, ate: ateInstant }),
    listarRecebimentosMlRecebidosNoPeriodo({ empresaId, desde, ate }),
  ]);
  ...
  let recebimentosMlRecebidoTotal = 0;
  for (const r of recebimentosMlRecebidos) {
    recebimentosMlRecebidoTotal = round2(recebimentosMlRecebidoTotal + (r.valorRecebido || 0));
  }
  ...
  for (const r of recebimentosMlRecebidos) {
    const dia = r.dataRecebimento;
    if (!dia) continue; // nunca deveria acontecer (status='recebido' sempre grava a data), defensivo
    mapaRecebidos.set(dia, round2((mapaRecebidos.get(dia) || 0) + (r.valorRecebido || 0)));
  }
  ...
  const aberturaDoPeriodo = await saldoDeAberturaEm(empresaId, saldoInicial, desde);
  const saldoAtual = await saldoDeAberturaEm(empresaId, saldoInicial, somarDiasData(hoje, 1));
  ...
  return {
    ...
    recebimentosMarketplaces: {
      // "total"/"quantidade" continuam significando PREVISTO (compatibilidade
      // com a tela — ver public/index.html), agora explicitamente sem os já
      // 'recebido' (senão duplicaria com o que entrou em realizadoNoPeriodo
      // acima). O que já foi recebido no período some daqui e passa a contar
      // só em realizadoNoPeriodo/serieDiaria, nunca nos dois ao mesmo tempo.
      total: recebimentosMlPrevistoTotal,
      quantidade: recebimentosMlPrevisto.length,
      recebidoNoPeriodo: recebimentosMlRecebidoTotal,
      quantidadeRecebida: recebimentosMlRecebidos.length,
