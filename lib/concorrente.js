...
async function buscarItemBaseDoProduto(empresaId, sku) {
  const { rows } = await pool.query(
    `SELECT pi.ml_item_id, pi.titulo, mc.id AS conta_id, mc.ml_user_id
...
async function buscarConcorrentesPorProduto({ empresaId, produtoId }) {
  ...
  return {
    modo,
    produto,
    itemBase: { mlItemId: itemBase.ml_item_id, titulo: tituloAtual },
    concorrentes,
    buscadoEm: new Date().toISOString(),
  };
}

module.exports = { buscarConcorrentesPorProduto };
