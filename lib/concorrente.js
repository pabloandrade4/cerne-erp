...
async function testarListagemPorVendedor({ empresaId, url }) {
  ...
  let item;
  let itemBuscadoSemLogin = false;
  try {
    item = await ml.apiGet(`/items/${itemId}`, accessToken);
  } catch (errAutenticado) {
    // Diagnóstico adicional (20/09/2026, feito depois do primeiro teste real
    // do usuário bloquear exatamente aqui): GET /items/{id} tradicionalmente
    // é público no Mercado Livre — dado de catálogo, sem precisar de login.
    // Tenta de novo SEM token, pra separar duas causas bem diferentes: (a) o
    // Mercado Livre bloqueia um vendedor de ver o anúncio de outro vendedor
    // pela API autenticada (aí a consulta pública ainda funcionaria), ou
    // (b) o bloqueio é do próprio anúncio/endpoint, independente de login
    // (aí a pública falha igual). NUNCA usada fora deste diagnóstico.
    try {
      item = await ml.apiGetPublico(`/items/${itemId}`);
      itemBuscadoSemLogin = true;
    } catch (errPublico) {
      return {
        ok: false, etapa: 'buscar_item', itemId,
        mensagem: 'A API do Mercado Livre recusou a consulta desse anúncio — tanto autenticado quanto sem login.',
        autenticado: { status: errAutenticado && errAutenticado.status, erro: (errAutenticado && errAutenticado.data) || (errAutenticado && errAutenticado.message) },
        semLogin: { status: errPublico && errPublico.status, erro: (errPublico && errPublico.data) || (errPublico && errPublico.message) },
      };
    }
  }
  ...
  let listagem;
  let listagemBuscadaSemLogin = false;
  try {
    listagem = await ml.apiGet(`/users/${sellerId}/items/search?status=active&limit=20`, accessToken);
  } catch (errAutenticado) {
    // Mesmo raciocínio do passo anterior (ver comentário acima): tenta de
    // novo sem token antes de desistir.
    try {
      listagem = await ml.apiGetPublico(`/users/${sellerId}/items/search?status=active&limit=20`);
      listagemBuscadaSemLogin = true;
    } catch (errPublico) {
      return {
        ok: false, etapa: 'listar_itens_do_vendedor', itemId, sellerId,
        mensagem: 'Deu pra identificar o vendedor, mas o Mercado Livre recusou listar os anúncios dele por esse caminho — tanto autenticado quanto sem login.',
        autenticado: { status: errAutenticado && errAutenticado.status, erro: (errAutenticado && errAutenticado.data) || (errAutenticado && errAutenticado.message) },
        semLogin: { status: errPublico && errPublico.status, erro: (errPublico && errPublico.data) || (errPublico && errPublico.message) },
      };
    }
  ...
  return {
    ...
    // Transparência sobre QUAL caminho funcionou (20/09/2026) — importa
    // pro próximo passo: se só funcionou sem login, o monitoramento
    // automático futuro não pode depender do token de nenhuma conta
    // conectada, e sim de chamadas públicas.
    itemBuscadoSemLogin,
    listagemBuscadaSemLogin,
    mensagem: 'Funcionou: deu pra listar os anúncios ativos desse vendedor só a partir do ID dele — esse caminho não depende da busca por título que está bloqueada.'
      + (itemBuscadoSemLogin || listagemBuscadaSemLogin ? ' (Precisou ser sem login em pelo menos uma das etapas — ver itemBuscadoSemLogin/listagemBuscadaSemLogin.)' : ''),
  };
}

// ============================================================================
// Cadastro manual de concorrente por SKU (20/09/2026)
// ============================================================================
// Ver comentário grande em db/schema.sql sobre a tabela
...
// anuncio do concorrente para ficar mais facil". Só CRUD simples, sem
// nenhuma chamada à API do Mercado Livre aqui — o link cadastrado nunca é
// validado/buscado automaticamente (ver motivo no schema); é o Agente
// Coordenador (lib/ia/coordenadorDiario.js, regra R4) que lê esta tabela
// pra anexar o link na conclusão do dia quando há um achado de anúncio
// pro mesmo SKU.
async function cadastrarConcorrente({ empresaId, sku, url, apelido }) {
  if (!sku || !String(sku).trim()) {
    const err = new Error('Informe o SKU do produto.');
    err.status = 400;
    throw err;
  }
  if (!url || !String(url).trim()) {
    const err = new Error('Informe o link do anúncio/loja do concorrente.');
    err.status = 400;
    throw err;
  }
  const { rows } = await pool.query(
    `INSERT INTO concorrentes_monitorados (empresa_id, sku, url, apelido)
          VALUES ($1, $2, $3, $4)
     ON CONFLICT (empresa_id, sku, url)
       DO UPDATE SET ativo = TRUE, apelido = COALESCE(EXCLUDED.apelido, concorrentes_monitorados.apelido), atualizado_em = now()
     RETURNING id, empresa_id, sku, url, apelido, ativo, criado_em, atualizado_em`,
    [empresaId, String(sku).trim(), String(url).trim(), apelido ? String(apelido).trim() : null]
  );
  return rows[0];
}

// Lista os concorrentes cadastrados de uma empresa — por padrão só os
// ativos (`incluirInativos` existe pra telas de histórico/gestão, mas o
// uso normal do Coordenador e da tela principal nunca precisa ver
// inativos).
async function listarConcorrentesMonitorados({ empresaId, sku, incluirInativos = false }) {
  const condicoes = ['empresa_id = $1'];
  const params = [empresaId];
  if (sku) {
    params.push(String(sku).trim());
    condicoes.push(`sku = $${params.length}`);
  }
  if (!incluirInativos) condicoes.push('ativo = TRUE');
  const { rows } = await pool.query(
    `SELECT id, empresa_id, sku, url, apelido, ativo, criado_em, atualizado_em
       FROM concorrentes_monitorados
      WHERE ${condicoes.join(' AND ')}
      ORDER BY sku ASC, criado_em DESC`,
    params
  );
  return rows;
}

// Usada pelo Coordenador (lib/ia/coordenadorDiario.js) — um mapa
// { [sku]: [{id, url, apelido}, ...] } com TODOS os concorrentes ativos da
// empresa, buscados de uma vez só por reunião do dia (nunca uma query por
// achado).
async function mapaConcorrentesMonitoradosPorSku(empresaId) {
  const linhas = await listarConcorrentesMonitorados({ empresaId });
  const mapa = {};
  for (const linha of linhas) {
    if (!mapa[linha.sku]) mapa[linha.sku] = [];
    mapa[linha.sku].push({ id: linha.id, url: linha.url, apelido: linha.apelido });
  }
  return mapa;
}

// "Remover" = desativar (nunca DELETE de verdade) — mesmo padrão de
// arquivamento usado no resto do sistema (radar_alertas.status,
// produtos.ativo), pra nunca perder rastro de qual concorrente já foi
// cadastrado antes, mesmo que o usuário decida parar de acompanhar.
async function removerConcorrenteMonitorado({ empresaId, id }) {
  const { rows } = await pool.query(
    `UPDATE concorrentes_monitorados
  ...
  if (!rows.length) {
    const err = new Error('Concorrente cadastrado não encontrado.');
    err.status = 404;
    throw err;
  }
  return rows[0];
...
module.exports = {
  ...
  cadastrarConcorrente, listarConcorrentesMonitorados,
