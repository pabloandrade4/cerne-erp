...
// Período anterior equivalente (20/09/2026, pedido explícito do usuário:
// "em visão geral quero colocar tudo que for possível... principalmente se
// o faturamento está caindo ou aumentando, comparado ao mês passado") —
// reaproveita EXATAMENTE a mesma definição já usada em Performance de
// Anúncios/Visitas e Conversão (ver lib/periodoComparacao.js: janela de
// MESMA DURAÇÃO imediatamente anterior ao período selecionado), nunca uma
// segunda definição de "período anterior" só pra esta tela.
...
// Saldo bancário consolidado (31/08/2026) — reaproveita EXATAMENTE
// lib/contasBancarias.js#saldoConsolidado, a mesma função já usada pela tela
// Contas Bancárias e por lib/fluxoCaixa.js (tela Fluxo de Caixa). Nenhuma
// fórmula nova: antes desta etapa o ERP realmente não tinha saldo bancário
// cadastrado, então este bloco sempre respondia null — agora que existe
// (ver "saldo bancário automático via extrato"), o resumo abaixo passa a
// mostrar o valor real quando disponível, em vez de manter uma afirmação
// que deixou de ser verdade.
const { saldoConsolidado } = require('./contasBancarias');
...
// ---------------- Por marketplace ----------------
// Até 14/09/2026, todo pedido vinha de ml_pedidos e o canal era sempre
// "Mercado Livre" (hardcoded). Desde a união com a Shopee em
// lib/relatorioVendas.js (buscarPedidosDoPeriodo agora traz `marketplace`
// já identificado em cada pedido — 'mercado_livre' | 'shopee'), esta função
// só lê esse campo — exatamente a extensão sem alterar mais nada desta
// tela já prevista no comentário original (pedido explícito do usuário:
// "quando houver outras integrações, elas devem entrar automaticamente").
function identificarCanal(pedido) {
  if (pedido.marketplace === 'shopee') return 'Shopee';
  // Venda de Balcão (22/09/2026) — 3º canal, mesmo espírito da extensão da
  // Shopee acima: sem esta branch explícita, toda venda de balcão cairia no
  // else e seria contada como "Mercado Livre" no painel "Vendas por
  // Marketplace" — dado errado, nunca só um rótulo feio.
  if (pedido.marketplace === 'balcao') return 'Venda no balcão';
...
// aumentando, comparado ao mês passado". Compara o faturamento do período
// selecionado no filtro do header com o período IMEDIATAMENTE ANTERIOR de
// MESMA DURAÇÃO (ver lib/periodoComparacao.js — mesma regra já usada em
// Performance de Anúncios/Visitas e Conversão, nunca uma definição nova).
// Quando falta faturamento em qualquer um dos dois lados (nenhum pedido no
// período, ou nenhum valor confirmado), a variação vem `null` com o motivo
// explícito — nunca uma % calculada a partir de um zero fabricado.
function evolucaoFaturamento({ faturamentoAtual, faturamentoAnterior, periodoAnterior }) {
  let variacaoValor = null;
  let variacaoPct = null;
  let tendencia = 'sem_dado';
  let motivoSemDado = null;

  if (faturamentoAtual === null || faturamentoAnterior === null) {
    motivoSemDado = 'Não há faturamento confirmado suficiente em um dos dois períodos para comparar.';
  } else {
    variacaoValor = round2(faturamentoAtual - faturamentoAnterior);
    variacaoPct = faturamentoAnterior !== 0 ? round2((variacaoValor / faturamentoAnterior) * 100) : null;
    if (variacaoPct === null && faturamentoAnterior === 0 && faturamentoAtual > 0) {
      // Sem base anterior (era 0) mas há faturamento agora — sobe, mas uma
      // % "infinita" não tem leitura útil: mostra a tendência sem número.
      tendencia = 'subindo';
    } else if (variacaoValor > 0) tendencia = 'subindo';
    else if (variacaoValor < 0) tendencia = 'caindo';
  ...
  return {
    ...
    tendencia,
    motivoSemDado,
    periodoAnterior: { desde: periodoAnterior.desdeStr, ate: periodoAnterior.ateStr },
  };
}
...
async function fluxoDeCaixa({ empresaId, desdeStr, ateStr, pedidos }) {
  const [contasAPagar, contasAReceber, saldoBancario] = await Promise.all([
    resumoContasPagar({ empresaId, desde: desdeStr, ate: ateStr }),
    resumoContasReceber({ empresaId, desde: desdeStr, ate: ateStr }),
    // Mesma proteção já usada em lib/fluxoCaixa.js: uma falha aqui (schema
    // bancário ainda incompatível, etc.) nunca pode derrubar o resto deste
    // painel — cai pra "não informado", nunca inventa.
    saldoConsolidado(empresaId).catch((err) => {
      console.warn('[visao-geral] não foi possível obter o saldo bancário consolidado:', err && err.message);
      return null;
    }),
  ]);
  ...
  return {
    ...
    contasAReceber,
    recebimentosMl,
    // saldoBancarioAtual = saldo consolidado (soma das contas bancárias com
    // saldo importado via extrato, ver lib/contasBancarias.js#saldoConsolidado
    // — MESMO número mostrado em Contas Bancárias/Fluxo de Caixa, nenhuma
    // fórmula nova). null quando nenhuma conta bancária tem saldo importado
    // ainda — nunca um valor inventado.
    saldoBancarioAtual: saldoBancario ? { valor: saldoBancario.valor, dataReferencia: saldoBancario.dataReferencia, motivo: null } : { valor: null, motivo: 'nenhuma_conta_bancaria_com_saldo_importado' },
    // saldoProjetado (fim do período à frente) precisa do cálculo dia-a-dia
    // completo de lib/fluxoCaixa.js#gerarFluxoDeCaixa (contas a pagar/
    // receber futuras dia a dia) — este resumo não duplica esse laço (pra
    // nunca virar uma segunda fórmula); por isso continua null aqui, com o
    // motivo correto (não é mais "sem saldo bancário cadastrado" — pode já
    // haver saldo bancário e mesmo assim a projeção dia-a-dia só existir na
    // tela Fluxo de Caixa).
    saldoProjetado: { valor: null, motivo: 'projecao_dia_a_dia_disponivel_na_tela_fluxo_de_caixa' },
  };
}
...
// ---------------- Função principal ----------------
async function painelVisaoGeral({ empresaId, periodoChave, desde: desdeQuery, ate: ateQuery }) {
  ...
  // Faturamento vs. período anterior (20/09/2026 — ver evolucaoFaturamento
  // acima). Busca só os PEDIDOS do período anterior (mesma fonte única de
  // sempre) — nunca uma segunda forma de somar faturamento.
  const periodoAnteriorCalc = periodoAnteriorEquivalente({ desde: periodoCalc.desde, ate: periodoCalc.ate });
  const { desde: periodoAnteriorDesdeStr, ate: periodoAnteriorAteStr } = periodoParaDatasBRT(periodoAnteriorCalc);
  const { pedidos: pedidosPeriodoAnterior } = await buscarPedidosDoPeriodo({
    empresaId, desde: periodoAnteriorCalc.desde, ate: periodoAnteriorCalc.ate,
  });
  const faturamentoEvolucao = evolucaoFaturamento({
    faturamentoAtual: canalResultado.totalFaturamento,
    faturamentoAnterior: resumirPeriodo(pedidosPeriodoAnterior).faturamento.valor,
    periodoAnterior: { desdeStr: periodoAnteriorDesdeStr, ateStr: periodoAnteriorAteStr },
  });
  ...
  // Resumo dos Agentes de IA (20/09/2026, pedido explícito do usuário: "em
  // visão geral quero colocar tudo que for possível... os agentes ia,
  // resumos") — reaproveita EXATAMENTE lib/ia/agentesResumo.js#obterResumoHub,
  // a MESMA função que já alimenta o hub "Agentes de IA" (kpis, pendentes e
  // sugestões novas de hoje por agente) — nenhum cálculo novo. Mesmo
  // require() lazy (dentro da função) e mesma proteção contra falha do
  // `radar` logo acima, e pelo mesmo motivo: agentesResumo.js também
  // depende de lib/ia/radar.js, então um require() no topo deste arquivo
  // fecharia a mesma dependência circular já documentada ali.
  let agentesIa = null, agentesIaError = null;
  try {
    const { obterResumoHub } = require('./ia/agentesResumo');
    agentesIa = await obterResumoHub(empresaId);
  } catch (e) {
    agentesIaError = 'Não foi possível carregar o resumo dos Agentes de IA agora.';
