...
async function conexoesEEmpresas(empresaId) {
  const [{ rows: totalEmpresasRows }, { rows: contasRows }] = await Promise.all([
  ...
  return {
    ...
    // Shopee ainda não foi integrada nesta etapa (fora do escopo pedido) —
    // mostrado como "nenhuma conta conectada", nunca um dado inventado, já
    // estruturado pra virar real automaticamente quando essa integração
    // existir (mesma ideia de "Por marketplace" acima).
    shopee: { contasConectadas: 0, status: 'nao_conectado' },
  };
}
