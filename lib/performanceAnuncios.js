...
const {
  ...
  buscarNomesProdutoPorSku, buscarContasFiltradas, diasEntre, calcularCrescimento,
...
async function gerarPerformanceAnuncios({ empresaId, contaId, sku, status, periodoCalc, desdeStr, ateStr }) {
  ...
  const linhas = [...chaves].map((chave) => {
    ...
    const mlItemId = (venda && venda.mlItemId) || (vivo && String(vivo.id)) || (chave.startsWith('sem-id:') ? null : chave);
    ...
    const sku = (venda && venda.sku) || (vivo && vivo.sku) || null;
    const statusAnuncio = vivo ? vivo.status : null;
    ...
    return {
      mlItemId,
      anuncio: (venda && venda.titulo) || (vivo && vivo.titulo) || null,
      sku,
