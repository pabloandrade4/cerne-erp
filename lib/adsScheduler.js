...
const { executarCicloDecisoesAds } = require('./ia/adsDecisoesCiclo');
...
async function executarCicloDeSincronizacaoAds({ sincronizarTodasAsContasAdsFn = sincronizarTodasAsContasAds } = {}) {
  ...
  // Agente de IA "Ads e Performance" Fase 1 (14/09/2026): depois de cada
  // sincronização, gera/atualiza as sugestões de ação em ia_decisoes_ads
  // (ver lib/ia/adsDecisoesCiclo.js) — só leitura do que acabou de ser
  // sincronizado, nunca uma chamada nova à API do Mercado Livre. Uma falha
  // aqui NUNCA deve mudar o resultado do ciclo de sincronização acima (que
  // já terminou com sucesso ou não, reportado em estado.ultimoCicloOk) —
  // por isso fica fora do try/catch principal e só loga o erro.
  try {
    await executarCicloDecisoesAds();
  } catch (err) {
    console.error('[Ads IA][decisões] ciclo de sugestões falhou:', err && err.message);
  }
