...
// Mesma lógica de contasPagar.js (somarDiasData) — deliberadamente
// duplicada aqui em vez de compartilhada, pra manter os dois módulos
// independentes um do outro (mesmo padrão já usado no resto do arquivo).
...
//
// "previstoProximos7Dias" — adicionado pra IA Gestora (mesma razão de
// contasPagar.js/vencendoProximos7Dias): recorte do saldo em aberto entre
// amanhã e 6 dias à frente, nunca um cálculo novo.
async function resumoContasReceber({ empresaId, desde, ate }) {
  ...
  let totalAReceber = 0, previstoHoje = 0, atrasado = 0, previstoProximos7Dias = 0;
  for (const r of abertos) {
    ...
    else if (prev <= limite7dias) previstoProximos7Dias += v;
  }
  ...
  return {
    ...
    previstoProximos7Dias: round2(previstoProximos7Dias),
...
// chamada diretamente pela tela de Contas a Receber. Mesma regra de
// sempre (nunca marca 2x, nunca "anda pra trás"): o UPDATE só afeta a
// linha se ainda não estiver recebida/cancelada — condição no próprio
// WHERE (atômico), igual ao padrão já usado em
// recebimentosMl.js#marcarComoRecebidoPorConciliacao. Recebe um `client`
// de transação (a confirmação de conciliação grava o movimento do
// extrato E a conta a receber juntos, ou nenhum dos dois).
async function marcarComoRecebidoPorConciliacao(client, id, dataRecebida) {
  const { rows } = await client.query(
    `UPDATE contas_receber SET status='recebido', data_recebida=$1, updated_at=now()
     WHERE id=$2 AND status NOT IN ('recebido','cancelado') RETURNING *`,
    [dataRecebida, id]
  );
