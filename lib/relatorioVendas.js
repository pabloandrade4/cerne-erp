...
// sem nenhuma fórmula nova. IMPORTANTE, mesma honestidade de sempre: a
// Shopee NÃO devolve comissão/tarifa de venda nos dados já sincronizados
// (get_order_detail) — isso só existe num endpoint de repasse/escrow da
// Shopee que este projeto ainda não implementa (Fase 2b, futura). Por isso,
// pra TODO pedido da Shopee: taxaVenda/pagamentoTaxas/pagamentoTaxaMarketplace
// sempre NULL (nunca inventados), o que automaticamente deixa
// `calculoCompleto`/margem como "pendente" (mesmo mecanismo já usado pra
// pedido do Mercado Livre sem custo de SKU cadastrado — ver
// lib/resultadoVenda.js). Faturamento bruto (valorTotal), custo do produto
// (por SKU, quando cadastrado) e frete continuam reais.
//
...
// Shopee, e o item da Shopee ainda não tem comissão própria pra ratear (ver
// nota acima). Ver o filtro explícito dentro da função.
...
// no topo do arquivo sobre o que ainda é NULL/pendente e por quê:
// comissão/tarifas da Shopee ainda não são buscadas — Fase 2b futura).
const SQL_SHOPEE_PEDIDOS_UNIFICADOS = `
  SELECT
    ...
    NULL::numeric(12,2) AS taxa_venda_total, -- comissão da Shopee: Fase 2b, ainda não buscada
    NULL::numeric(12,2) AS pagamento_taxas,
    NULL::numeric(12,2) AS pagamento_taxa_marketplace,
