...
// docs/02-decisoes.md e docs/PROPOSTA-contexto-negocio-ia-gestora.md): 2
// ferramentas novas, ADITIVAS, ambas SOMENTE LEITURA — identificar_produto_fisico
// (lib/mapaProdutos.js, fora de lib/ia/ por decisão do usuário — ponto 1 da
// proposta) e visao_geral_empresa (lib/ia/contextoNegocio.js, composição em
// cima de lib/visaoGeralPainel.js/lib/recebimentosMl.js, nenhum cálculo
// novo). Nenhuma ferramenta já existente foi alterada.
const { identificarProdutoFisico } = require('../mapaProdutos');
const { montarRaioXEmpresa } = require('./contextoNegocio');
...
// ---------------- Etapa (b): contexto de negócio ----------------

// Identifica um produto FÍSICO (produto base — caixa/kit, não o SKU de
// venda) a partir de um texto livre do usuário (ex: "a caixa 20x20x20",
// "CX-19X12X12", um apelido cadastrado em Mapa de Produtos > Apelidos).
// Casca fina sobre lib/mapaProdutos.js#identificarProdutoFisico — NUNCA
// escolhe sozinha entre produtos ambíguos (ver regra central naquele
// arquivo): quando `status` vier 'ambiguo', a resposta traz os candidatos e
// a IA deve PERGUNTAR ao usuário qual é o certo, nunca supor.
async function handleIdentificarProdutoFisico(input, ctx) {
  const textoLivre = input && input.textoLivre;
  if (!textoLivre || !String(textoLivre).trim()) {
    return { erro: 'Informe o texto (código, apelido ou medida) do produto a identificar.' };
  }
  const resultado = await identificarProdutoFisico(ctx.empresaId, textoLivre);

  const observacoesPorStatus = {
    identificado: 'Produto identificado com certeza — pode usar o campo "codigo" dele como referência em outras ferramentas que aceitam filtro de produto (ex: estoque_fisico_detalhado > produtoBase).',
    ambiguo: 'Mais de um produto bate com esse texto — NUNCA escolha um sozinho: liste os candidatos (campo "candidatos") para o usuário e pergunte qual ele quer dizer antes de continuar.',
    nao_encontrado: 'Nenhum produto cadastrado em Mapa de Produtos bate com esse texto, nem de forma aproximada — diga isso claramente ao usuário; não invente um produto nem assuma um código parecido.',
  };

  return {
    ...resultado,
    observacao: observacoesPorStatus[resultado.status] || observacoesPorStatus.nao_encontrado,
  };
}

// "Raio-X" da empresa — visão ampla numa única chamada, pensada pra
// perguntas gerais/abertas ("como está o negócio", "me dê um resumo geral",
// "tem algo que eu deveria olhar hoje"), evitando a IA precisar encadear 4-5
// ferramentas separadas. Casca de COMPOSIÇÃO sobre lib/ia/contextoNegocio.js
// (que por sua vez só reaproveita lib/visaoGeralPainel.js — mesma fonte da
// tela Visão Geral — e lib/recebimentosMl.js) — nenhum cálculo novo. Para
// uma pergunta específica de um único número, prefira sempre a ferramenta
// dedicada daquele número (ex: fluxo_de_caixa_detalhado, contas_a_receber_resumo,
// recebimentos_marketplace_resumo) — elas trazem mais detalhe e custam menos.
async function handleVisaoGeralEmpresa(_input, ctx) {
  const raioX = await montarRaioXEmpresa({ empresaId: ctx.empresaId, periodoChave: ctx.periodoCalc.chave });

  const alertasResumo = (raioX.alertas || []).map((a) => ({ tipo: a.tipo, severidade: a.severidade, titulo: a.titulo, descricao: a.descricao, telaRelacionada: a.pagina }));

  const conexoes = raioX.conexoes || {};
  const radarDisponivel = !raioX.radarError && raioX.radar;

  return {
    periodo: raioX.periodo,
    vendasPorCanal: {
      totalFaturamento: campoResposta({ valor: (raioX.porCanal && raioX.porCanal.totalFaturamento) || null }),
      // `l.faturamento` já vem no formato {valor,pendentes} (mesmo formato de
      // resumirPeriodo — ver lib/visaoGeralPainel.js#porCanal), nunca um
      // número puro — por isso campoResposta aqui, igual ao resto do arquivo.
      linhas: ((raioX.porCanal && raioX.porCanal.linhas) || []).map((l) => ({
        canal: l.canal,
        quantidadePedidos: l.qtdPedidos,
        faturamento: campoResposta(l.faturamento),
        participacaoPercentual: l.participacaoPercentual,
      })),
    },
    fluxoCaixa: {
      contasAPagar: raioX.fluxoCaixa && raioX.fluxoCaixa.contasAPagar,
      contasAReceber: raioX.fluxoCaixa && raioX.fluxoCaixa.contasAReceber,
      recebimentosMercadoLivreEsperadosNoPeriodo: raioX.fluxoCaixa
        ? campoResposta({ valor: raioX.fluxoCaixa.recebimentosMl.valorLiquidoEsperado, pendentes: raioX.fluxoCaixa.recebimentosMl.pendentes })
        : null,
    },
    recebimentosMarketplacePorStatus: raioX.recebimentosMarketplacePorStatus,
    conexoes: {
      totalEmpresas: conexoes.empresas && conexoes.empresas.total,
      mercadoLivre: conexoes.mercadoLivre,
      shopee: conexoes.shopee,
    },
    quantidadeDeAlertas: alertasResumo.length,
    alertas: alertasResumo,
    radar: radarDisponivel ? raioX.radar : null,
    radarIndisponivel: !radarDisponivel,
    observacao: 'Visão ampla da empresa selecionada, no período do cabeçalho, composta a partir das MESMAS fontes já usadas em Visão Geral (vendas por canal, fluxo de caixa, conexões, alertas, Radar) e em Recebimentos (detalhe por status) — nenhum número novo. Use esta ferramenta para uma pergunta geral/aberta sobre o negócio; para um único número específico, prefira sempre a ferramenta dedicada daquele número (ex: fluxo_de_caixa_detalhado, contas_a_receber_resumo, recebimentos_marketplace_resumo), que trazem mais detalhe.',
  };
}
...
const FERRAMENTAS = [
  ...
  {
    name: 'identificar_produto_fisico',
    description: 'Identifica um produto FÍSICO (produto base — a caixa/kit cadastrado em Mapa de Produtos, não o SKU de venda) a partir de um texto livre: código (ex: "CX-19X12X12"), apelido cadastrado, ou medida (ex: "20x20x20"). Devolve um de 3 status: "identificado" (achou exatamente 1 — pode usar o código dele em outras ferramentas), "ambiguo" (mais de um produto bate — SEMPRE pergunte ao usuário qual ele quer dizer, listando os candidatos; nunca escolha um sozinho) ou "nao_encontrado". Use antes de responder uma pergunta sobre um produto físico específico mencionado de forma informal (ex: "quanto tenho da caixa 20x20x20", "o produto X vendeu bem essa semana?") quando não tiver certeza do código exato.',
    input_schema: {
      type: 'object',
      properties: {
        textoLivre: { type: 'string', description: 'Como o usuário se referiu ao produto na pergunta (código, apelido ou medida).' },
      },
      required: ['textoLivre'],
      additionalProperties: false,
    },
    handler: handleIdentificarProdutoFisico,
  },
  {
    name: 'visao_geral_empresa',
    description: 'Visão ampla ("raio-X") da empresa e período selecionados numa única chamada: vendas por canal/loja, fluxo de caixa (contas a pagar/receber em aberto, recebimentos de marketplace esperados e por status), conexões com marketplaces (Mercado Livre/Shopee), alertas operacionais ativos e o resumo do Radar. Use para perguntas GERAIS/ABERTAS como "como está o negócio", "me dê um resumo geral/executivo", "tem algo que eu deveria olhar hoje", "está tudo bem com minhas vendas/conexões". Para uma pergunta sobre um único número específico, prefira sempre a ferramenta dedicada (ex: fluxo_de_caixa_detalhado, contas_a_receber_resumo, recebimentos_marketplace_resumo, alertas_operacionais) — elas trazem mais detalhe.',
    input_schema: { type: 'object', properties: {}, additionalProperties: false },
    handler: handleVisaoGeralEmpresa,
  },
