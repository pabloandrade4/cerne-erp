...
function adEstoqueValorParado(s) {
  if (!s) return null;
  return {
    kpis: [
      kpi('Valor estoque Full', numOrNull(s.valorEstoqueFull), s.valorEstoqueFull && s.valorEstoqueFull.valorFormatado, { grupo: 'FULL' }),
      kpi('Valor estoque normal', numOrNull(s.valorEstoqueNormal), s.valorEstoqueNormal && s.valorEstoqueNormal.valorFormatado, { grupo: 'FORA DO FULL' }),
      kpi('Valor total (Full + normal)', numOrNull(s.valorTotal), s.valorTotal && s.valorTotal.valorFormatado),
    ],
  };
}

function adEstoqueFisicoDetalhado(s) {
  if (!s) return null;
  const bloco = s.full || s.foraDoFull;
  if (!bloco || !Array.isArray(bloco.produtos) || !bloco.produtos.length) return null;
  const top = bloco.produtos.slice(0, 8);
  const tituloBloco = s.escopo === 'ambos' ? 'Estoque físico' : (s.escopo === 'fora_do_full' ? 'Estoque (fora do Full)' : 'Estoque Full');
  return {
    kpis: [
      kpi('Valor total a custo', bloco.valorTotalACusto, bloco.valorTotalACustoFormatado),
      kpi('Unidades físicas', bloco.unidadesFisicas, bloco.unidadesFisicas === null || bloco.unidadesFisicas === undefined ? null : `${bloco.unidadesFisicas} caixas`),
    ],
    tabela: {
      titulo: tituloBloco,
      colunas: [
        { header: 'Produto', key: 'produto', tipo: 'texto' },
        { header: 'Unidades físicas', key: 'unidades', tipo: 'int' },
        { header: 'Custo unitário', key: 'custo', tipo: 'money' },
        { header: 'Valor em estoque', key: 'valor', tipo: 'money' },
      ],
      linhas: bloco.produtos.map((p) => ({ produto: p.produto, unidades: p.unidadesFisicas, custo: p.custoUnitario, valor: p.valorEmEstoque })),
    },
    graficos: [{
      tipo: 'barras',
      titulo: `Valor em estoque por produto — ${tituloBloco}`,
      categorias: top.map((p) => p.produto),
      series: [{ nome: 'Valor em estoque', tipo: 'money', valores: top.map((p) => p.valorEmEstoque || 0) }],
    }],
  };
}
...
function adRecebimentosMarketplaceResumo(s) {
  if (!s) return null;
  return {
    kpis: [
      kpi('Recebido hoje', numOrNull(s.recebidoHoje), s.recebidoHoje.valorFormatado, { grupo: 'REALIZADO' }),
      kpi('Recebido este mês', numOrNull(s.recebidoEsteMes), s.recebidoEsteMes.valorFormatado, { grupo: 'REALIZADO' }),
      kpi('A receber (total)', numOrNull(s.aReceberTotal), s.aReceberTotal.valorFormatado, { grupo: 'PROJETADO' }),
      kpi('A receber atrasado', numOrNull(s.aReceberAtrasado), s.aReceberAtrasado.valorFormatado, { grupo: 'PROJETADO' }),
    ],
    tabela: Array.isArray(s.porMarketplace) && s.porMarketplace.length ? {
      titulo: 'Recebimentos por marketplace',
      colunas: [
        { header: 'Marketplace', key: 'marketplace', tipo: 'texto' },
        { header: 'Recebido', key: 'recebido', tipo: 'money' },
        { header: 'A receber', key: 'aReceber', tipo: 'money' },
      ],
      linhas: s.porMarketplace.map((m) => ({ marketplace: m.marketplace, recebido: m.recebido, aReceber: m.aReceber })),
    } : null,
  };
}

function adFluxoDeCaixaDetalhado(s) {
  if (!s) return null;
  const saldoAtual = numOrNull(s.saldoAtual);
  const entradasMl = s.entradasPrevistas && s.entradasPrevistas.recebimentosMarketplacesPrevistos;
  const entradasCr = s.entradasPrevistas && s.entradasPrevistas.contasAReceberNoPeriodo;
  return {
    kpis: [
      ...(s.saldoAtual && s.saldoAtual.disponivel ? [kpi('Caixa atual (saldo realizado)', saldoAtual, s.saldoAtual.valorFormatado, { grupo: 'REALIZADO' })] : []),
      kpi('Entradas realizadas no período', numOrNull(s.realizadoNoPeriodo.entradas), s.realizadoNoPeriodo.entradas.valorFormatado, { grupo: 'REALIZADO' }),
      kpi('Saídas realizadas no período', numOrNull(s.realizadoNoPeriodo.saidas), s.realizadoNoPeriodo.saidas.valorFormatado, { grupo: 'REALIZADO' }),
      kpi('Entradas previstas (marketplace)', entradasMl ? numOrNull(entradasMl) : null, entradasMl ? entradasMl.valorFormatado : null, { grupo: 'PROJETADO' }),
      kpi('Entradas previstas (Contas a Receber)', entradasCr ? numOrNull(entradasCr) : null, entradasCr ? entradasCr.valorFormatado : null, { grupo: 'PROJETADO' }),
      kpi('Saídas previstas', numOrNull(s.saidasPrevistas), s.saidasPrevistas.valorFormatado, { grupo: 'PROJETADO' }),
      ...(s.saldoProjetado && s.saldoProjetado.disponivel ? [kpi('Saldo projetado', numOrNull(s.saldoProjetado), s.saldoProjetado.valorFormatado, { grupo: 'PROJETADO' })] : []),
    ],
  };
}

function adExtratoBancarioAnalise(s) {
  if (!s || !s.temMovimentacaoNoPeriodo) return null;
  return {
    kpis: [
      kpi('Entradas no extrato', numOrNull(s.totalEntradas), s.totalEntradas.valorFormatado, { grupo: 'REALIZADO' }),
      kpi('Saídas no extrato', numOrNull(s.totalSaidas), s.totalSaidas.valorFormatado, { grupo: 'REALIZADO' }),
      kpi('Conciliado', numOrNull(s.recebimentosConciliados), s.recebimentosConciliados.valorFormatado, { grupo: 'REALIZADO' }),
      kpi('Ainda não identificado', numOrNull(s.naoIdentificados), s.naoIdentificados.valorFormatado),
    ],
    tabela: Array.isArray(s.principaisEntradas) && s.principaisEntradas.length ? {
      titulo: 'Principais entradas do extrato',
      colunas: [
        { header: 'Data', key: 'data', tipo: 'texto' },
        { header: 'Descrição', key: 'descricao', tipo: 'texto' },
        { header: 'Valor', key: 'valor', tipo: 'money' },
      ],
      linhas: s.principaisEntradas.map((m) => ({ data: m.data, descricao: m.descricao, valor: m.valor })),
    } : null,
  };
}
...
const ADAPTADORES = {
  ...
  estoque_valor_parado: adEstoqueValorParado,
  estoque_fisico_detalhado: adEstoqueFisicoDetalhado,
  recebimentos_marketplace_resumo: adRecebimentosMarketplaceResumo,
  fluxo_de_caixa_detalhado: adFluxoDeCaixaDetalhado,
  extrato_bancario_analise: adExtratoBancarioAnalise,
};
