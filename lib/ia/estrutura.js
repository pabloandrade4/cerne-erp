...
function adDreCompleta(s) {
  ...
  const linhasTabela = [
    ...
    ['Margem de contribuição', l.margemDeContribuicao], ['Despesas do período', l.despesasDoPeriodo],
    ['Resultado final', l.resultadoFinal],
  ];
  const porCategoria = Array.isArray(s.despesasPorCategoria) ? s.despesasPorCategoria.slice(0, 8) : [];
  return {
    kpis: [
    ...
    // "Para onde está indo o dinheiro?" — mesmos números do bloco por
    // categoria da tela DRE, nunca uma segunda consulta.
    graficos: porCategoria.length ? [{
      tipo: 'barras',
      titulo: 'Despesas por categoria',
      categorias: porCategoria.map((g) => g.categoria),
      series: [{ nome: 'Despesas', tipo: 'money', valores: porCategoria.map((g) => g.total || 0) }],
    }] : [],
  };
}

function adDespesasPorCategoria(s) {
  if (!s || !Array.isArray(s.despesas) || !s.despesas.length) return { kpis: [kpi('Despesas encontradas', s && s.quantidadeEncontrada || 0, String(s && s.quantidadeEncontrada || 0))] };
  return {
    kpis: [
      kpi('Total encontrado', s.total, s.totalFormatado),
      kpi('Lançamentos', s.quantidadeEncontrada, String(s.quantidadeEncontrada)),
    ],
    tabela: {
      titulo: s.categoriaFiltrada ? ('Despesas — ' + s.categoriaFiltrada) : 'Despesas',
      colunas: [
        { header: 'Data', key: 'data', tipo: 'texto' }, { header: 'Descrição', key: 'descricao', tipo: 'texto' },
        { header: 'Categoria', key: 'categoria', tipo: 'texto' }, { header: 'Fornecedor', key: 'fornecedor', tipo: 'texto' },
        { header: 'Valor', key: 'valor', tipo: 'money' },
      ],
      linhas: s.despesas.map((d) => ({ data: d.data, descricao: d.descricao, categoria: d.subcategoria ? (d.categoria + ' › ' + d.subcategoria) : d.categoria, fornecedor: d.fornecedor, valor: d.valor })),
    },
  };
}
...
const ADAPTADORES = {
  ...
  despesas_por_categoria: adDespesasPorCategoria,
