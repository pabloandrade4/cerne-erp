...
async function buscarConfigPromocoes(empresaId) {
  const { rows } = await pool.query('SELECT margem_minima_pct, margem_conforto_pct, dias_cobertura_alta FROM config_promocoes WHERE empresa_id = $1', [empresaId]);
  if (!rows.length) return { margemMinimaPct: 14, margemConfortoPct: 0, diasCoberturaAltaLimite: 60 };
  return {
    margemMinimaPct: Number(rows[0].margem_minima_pct),
    margemConfortoPct: Number(rows[0].margem_conforto_pct) || 0,
    diasCoberturaAltaLimite: Number(rows[0].dias_cobertura_alta) || 60,
  };
}

// "Estoque alto" (20/09/2026, pedido explícito do usuário — ver
// lib/promocoesMotor.js#calcularCoberturaEstoque). Soma o estoque REAL já
// sincronizado por `ml_estoque_itens` (MESMA tabela/fonte da tela Estoque —
// nunca uma segunda sincronização própria daqui), por SKU, pra toda a
// empresa (fora do Full + Full somados, é o estoque físico total do
// produto). `pendente = FALSE` porque uma linha pendente não tem
// quantidade confiável (ver comentário da tabela em db/schema.sql) — soma
// como se não existisse, nunca como zero.
async function buscarEstoqueAtualPorSku(empresaId) {
  const { rows } = await pool.query(
    `SELECT sku, SUM(quantidade) AS total
       FROM ml_estoque_itens
      WHERE empresa_id = $1 AND sku IS NOT NULL AND pendente = FALSE AND quantidade IS NOT NULL
      GROUP BY sku`,
    [empresaId]
  );
  const mapa = new Map();
  rows.forEach((r) => mapa.set(r.sku, Number(r.total)));
  return mapa;
}
...
async function upsertAnalise(linha) {
  await pool.query(
    `INSERT INTO promocoes_analises (
       ...
       margem_real, margem_real_pct, margem_normal_pct, margem_minima_pct_usada,
       margem_conforto_pct_usada, tem_desconto,
       margem_incompleta, motivo_incompleto,
       estoque_atual, vendas_unidades_90d, cobertura_dias_estoque, estoque_alto,
     ...
     ON CONFLICT (conta_id, promotion_id, ml_item_id) DO UPDATE SET
       ...
       margem_normal_pct = EXCLUDED.margem_normal_pct,
       ...
       vendas_unidades_90d = EXCLUDED.vendas_unidades_90d,
       cobertura_dias_estoque = EXCLUDED.cobertura_dias_estoque,
       estoque_alto = EXCLUDED.estoque_alto,
    ...
    [
      ...
      linha.margemReal, linha.margemRealPct, linha.margemNormalPct, linha.margemMinimaPctUsada,
      linha.margemConfortoPctUsada, !!linha.temDesconto,
      linha.margemIncompleta, linha.motivoIncompleto,
      linha.estoqueAtual, linha.unidadesVendidas90d, linha.coberturaDiasEstoque, !!linha.estoqueAlto,
...
async function executarCicloPromocoesConta({ conta, custoPorSku, historicoPorSku, aliquotaImposto, margemMinimaPct, margemConfortoPct, estoqueAtualPorSku, diasCoberturaAltaLimite }) {
...
async function executarCicloPromocoesEmpresa(empresaId) {
  ...
  const [configPromocoes, aliquotaImposto, custoPorSku, estoqueAtualPorSku] = await Promise.all([
    buscarConfigPromocoes(empresaId),
    buscarAliquotaImposto(empresaId),
    buscarCustoPorSku(empresaId),
    buscarEstoqueAtualPorSku(empresaId),
  ]);
  const { margemMinimaPct, margemConfortoPct, diasCoberturaAltaLimite } = configPromocoes;
  ...
  for (const conta of contas) {
    try {
      const resultadoConta = await executarCicloPromocoesConta({ conta, custoPorSku, historicoPorSku, aliquotaImposto, margemMinimaPct, margemConfortoPct, estoqueAtualPorSku, diasCoberturaAltaLimite });
...
module.exports = {
  ...
  // Exportados só pra teste de integração direto (20/09/2026, feature
  // "estoque alto") — nunca chamados de fora do ciclo em código de produção.
  buscarEstoqueAtualPorSku,
  upsertAnalise,
};
