...
async function upsertDecisaoPromocao(linha, sugestao) {
  ...
  if (existente.rows.length) {
    ...
    await pool.query(
      `UPDATE ia_decisoes_promocoes SET
         ...
         valor_sugerido_ia = $14, atualizado_em = now()
       WHERE id = $15`,
      [
  ...
  const { rows } = await pool.query(
    `INSERT INTO ia_decisoes_promocoes (
       ...
       snapshot_margem_real, snapshot_margem_real_pct, snapshot_margem_normal_pct, valor_sugerido_ia
    ...
    [
      ...
      linha.precoNormal, linha.precoPromo, linha.descontoPct,
      linha.custoProduto, linha.tarifasEstimadas, linha.freteVendedorEstimado, linha.impostoEstimado,
      linha.margemReal, linha.margemRealPct, linha.margemNormalPct, JSON.stringify(sugestao.valorSugeridoIa || {}),
    ]
  );
  return rows[0].id;
}
