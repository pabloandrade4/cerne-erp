...
const TEMAS = {
  ...
  shopee: {
    titulo: 'Shopee',
    texto: 'A integração com a Shopee ainda não foi desenvolvida neste ERP — hoje só existe integração real com o Mercado Livre. Qualquer pergunta sobre dados da Shopee deve ser respondida com honestidade: não há dado disponível ainda, não uma estimativa.',
  },
  permissoes_usuario: {
    titulo: 'Permissões de usuário e login',
    texto: 'O ERP ainda não tem login nem permissão por usuário implementados (existe a tabela "users" no banco, mas sem tela nem rota de uso). Por isso, hoje, o único controle de acesso que existe em qualquer parte do sistema — inclusive na IA Gestora — é a própria empresa selecionada precisar existir de verdade. Quando login/permissão por usuário existir, é aqui (na camada de ferramentas da IA) que essa checagem adicional será aplicada.',
  },
};
