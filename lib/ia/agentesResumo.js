...
//
// 20/09/2026, pedido explícito do usuário ("quando clicar em cima do
// agente, quero ver... oque ele fez"): os alertas do Radar (radar_alertas)
// eram TODOS gravados aqui com agente_codigo='radar', mesmo os que já
// pertencem a um agente real e cadastrado (anuncio_% é do agente
// "anuncios_radar", concorrente_ativo é do agente "concorrente") — por
// isso o histórico filtrado POR AGENTE nunca mostrava nada pra esses dois.
// Corrigido pra atribuir pela categoria real; o que sobra (financeiro_*,
// estoque_*, custo_alterado etc.) não pertence a nenhum agente cadastrado
// ainda (ver EM_CONSTRUCAO em public/index.html) — fica como
// 'radar_negocio', nunca inventado como se already fosse um agente real.
// `agenteCodigo` (novo parâmetro, opcional) filtra pra um agente só — usado
// pelo card "Detalhe do Agente" (ver lib/ia/agenteDetalhe.js); omitido,
// continua devolvendo TODOS os eventos, exatamente como antes (aba
// "Histórico" do hub). ('radar_negocio' nunca aparece como se já fosse um
// agente real cadastrado — só existe pra não perder o evento.)
async function listarHistorico(empresaId, limit, agenteCodigo) {
  ...
  const { rows } = await pool.query(
    ...
    (
     ...
     UNION ALL
     (
       -- Atribuição pela categoria real (ver comentário grande acima da
       -- função) — nunca mais um "radar" genérico escondendo o agente de
       -- verdade por trás do alerta.
       SELECT criado_em AS ts,
              (CASE WHEN categoria LIKE 'anuncio_%' THEN 'anuncios_radar'
                    WHEN categoria = 'concorrente_ativo' THEN 'concorrente'
                    ELSE 'radar_negocio' END)::text AS agente_codigo,
              'alerta'::text AS tipo,
     ...
     WHERE ($3::text IS NULL OR t.agente_codigo = $3)
     ORDER BY ts DESC
     LIMIT $2`,
    [empresaId, lim, agenteCodigo || null]
  );
  return rows.map((r) => ({
    ts: r.ts,
