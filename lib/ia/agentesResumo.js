...
async function obterResumoHub(empresaId) {
  const [{ rows: agentesRows }, ads, promocoes, iaGestora, alertasNovosHoje, radar] = await Promise.all([
  ...
  const statsPorCodigo = { ads_performance: ads, promocoes };
  ...
  // Performance + Promoções) + novos alertas detectados pelo Radar. É uma
  // soma de fontes diferentes de propósito (nenhuma delas sozinha
  // representa "o que os agentes fizeram hoje") — nunca inclui nada que a
  // IA ainda não fez de verdade (aprovações são AÇÃO DO USUÁRIO, não do
  // agente, por isso não entram aqui).
  const tarefasHoje = ads.sugestoesNovasHoje + promocoes.sugestoesNovasHoje + alertasNovosHoje;
