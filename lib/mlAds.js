...
async function buscarAdsComMetricas({ accessToken, siteId, advertiserId, desde, ate, aggregationType, limit = 100, offset = 0 }) {
  ...
  // CORREÇÃO (11/09/2026, evidência real em produção — conta
  // PFEMBALAGEMS/PLACKBOX, advertiser_id 753060, 76 anúncios ativos
  // confirmados): faltava o sufixo `/search` no final — mesma mudança que a
  // Mercado Livre já tinha feito antes no endpoint de CAMPANHAS (ver
  // `buscarCampanhasNovo` abaixo, "From now, the request to
  // .../product_ads/campaigns must include /search"), só que pro endpoint de
  // ITENS essa mesma exigência nunca tinha sido testada contra uma conta com
  // Product Ads ativo — sem o sufixo, a API respondia 404 com a mensagem
  // genérica de rota inexistente ("...visita o Sitio de Desarrolladores...",
  // não uma lista vazia). Testado ao vivo: com `/search`, retornou os 76
  // anúncios reais da conta, com métricas (clicks, cost, ROAS, ACOS, etc.)
  // batendo com o painel do Mercado Livre.
  const endpoint = `/marketplace/advertising/${siteId}/advertisers/${advertiserId}/product_ads/ads/search?${qs.toString()}`;
  try {
...
// conta PFEMBALAGEMS tinha advertiser_id confirmado, mas o endpoint "novo"
// respondia 404). CAUSA REAL ENCONTRADA EM 11/09/2026: o problema não era o
// formato novo-vs-clássico — era que o endpoint "novo" estava sem o sufixo
// `/search` no final (ver a correção em `buscarAdsComMetricas` acima).
// Testado ao vivo contra a conta PFEMBALAGEMS/PLACKBOX (advertiser_id
// 753060, 76 anúncios ativos): com `/search`, o formato novo funciona
// perfeitamente — não foi mais preciso usar o clássico. Este fallback fica
// mantido por segurança (uma conta diferente pode legitimamente precisar
// dele), mas na prática o formato novo (com `/search`) deve resolver a
// grande maioria dos casos agora.
...
// PRIMEIRA ação de ESCRITA de verdade no Mercado Livre deste projeto
// (19/09/2026, pedido explícito do usuário: "eu vou aprovar, aí vai fazer...
// por enquanto só vai precisar da minha permissão" — a IA de Ads passa a
// EXECUTAR de verdade uma sugestão já aprovada por ele, em vez de só marcar
// "aprovada" no banco como fazia até aqui). Endpoint e formato confirmados
// na documentação oficial atual (global-selling.mercadolibre.com/devsite/
// new-product-ads, seção "Update a campaign", 19/09/2026):
//   PUT /marketplace/advertising/{site_id}/product_ads/campaigns/{campaign_id}
//   headers: Authorization Bearer + Api-Version: 2
//   body: { budget?, status?, name?, acos_target?, strategy? } — só manda o
//   que for de fato alterar (a própria documentação pede "ao menos um dos
//   parâmetros que deseja atualizar").
// AINDA NÃO TESTADO CONTRA A API DE VERDADE (mesma limitação de sempre
// deste ambiente de desenvolvimento — sem acesso a api.mercadolibre.com).
// Se algum nome de campo tiver mudado, o erro real da API aparece em
// err.data/err.status pra quem chamou (lib/ia/adsExecutor.js) gravar —
// nunca finge sucesso.
async function atualizarCampanha({ accessToken, siteId, campanhaId, budget, status }) {
  const body = {};
  if (budget !== undefined && budget !== null) body.budget = budget;
  if (status !== undefined && status !== null) body.status = status;
  if (!Object.keys(body).length) {
    throw new Error('atualizarCampanha: nada pra atualizar (nem budget nem status foram passados).');
  }
  const endpoint = `/marketplace/advertising/${siteId}/product_ads/campaigns/${campanhaId}`;
  try {
    return await ml.apiPut(endpoint, accessToken, JSON.stringify(body), { 'Api-Version': '2' });
  } catch (err) {
    err.contexto = { endpoint, parametros: body };
    throw err;
  }
}
