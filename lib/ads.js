...
async function sincronizarContaAds(contaId) {
  ...
  for (const chave of chaves) {
    ...
    if (!resultado.disponivel) {
      ...
      // DIAGNÓSTICO TEMPORÁRIO (11/09/2026): loga o corpo REAL da resposta
      // do Mercado Livre (dos dois formatos de endpoint, quando os dois
      // foram tentados) pra investigar por que a conta PFEMBALAGEMS continua
      // recebendo 404 mesmo após a correção do endpoint clássico em
      // lib/mlAds.js. Remover depois que o motivo real for confirmado —
      // nunca aparece pro usuário, só no log do servidor (Render).
      if (resultado.motivo === 'sem_anuncios_ads' || resultado.motivo === 'erro_api' || resultado.motivo === 'parametro_invalido') {
        console.error(`[Ads][diagnóstico] conta ${contaId} janela ${chave}: motivo=${resultado.motivo} detalheApi=${JSON.stringify(resultado.detalheApi)}`);
      }
