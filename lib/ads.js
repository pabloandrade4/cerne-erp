...
async function sincronizarContaAds(contaId) {
  ...
  try {
    ...
    await upsertAdsConta(contaId, { disponivel: false, motivo: 'erro_api', mensagem: 'Não foi possível ler o token de acesso desta conta.' });
    return { ok: false };
  }
...
async function sincronizarTodasAsContasAds() {
  ...
  const resultados = await Promise.allSettled(contas.map((c) => sincronizarContaAds(c.id)));
  const comErro = [];
