...
// Lista contas a pagar de uma empresa, filtrando por vencimento dentro do
// período [desde, ate] (datas BRT, 'YYYY-MM-DD', inclusive nas duas
// pontas) e, opcionalmente, por status efetivo e por busca livre
// (descrição, categoria ou fornecedor).
async function listarContasPagar({ empresaId, desde, ate, status, search }) {
  const conditions = ['cp.empresa_id = $1', 'cp.vencimento >= $2', 'cp.vencimento <= $3'];
...
async function resumoContasPagar({ empresaId, desde, ate }) {
  ...
  let totalAPagar = 0, vencendoHoje = 0, vencidas = 0;
