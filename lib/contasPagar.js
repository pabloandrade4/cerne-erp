...
//
// Regra de imutabilidade: uma conta já PAGA não pode mais ser editada nem
// excluída (só existe daqui pra frente como registro histórico — igual
// contabilidade de verdade: se pagou errado, cancela um novo lançamento
// de estorno, não edita o que já virou fato financeiro). Uma conta
// CANCELADA também não pode mais ser editada. Só uma conta PENDENTE
// (vencida ou não) pode ser editada, cancelada ou excluída.
...
async function listarContasPagar({ empresaId, desde, ate, status, search }) {
  const conditions = ['cp.empresa_id = $1', "(cp.status != 'pago' OR (cp.data_pagamento >= $2 AND cp.data_pagamento <= $3))"];
  const params = [empresaId, desde, ate];
  if (search && search.trim()) {
    params.push('%' + search.trim() + '%');
    const idx = params.length;
    conditions.push('(cp.descricao ILIKE $' + idx + ' OR cp.categoria ILIKE $' + idx + ' OR f.razao_social ILIKE $' + idx + ')');
  }
...
async function criarContaPagar(body) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO contas_pagar (empresa_id, fornecedor_id, descricao, categoria, valor, vencimento, observacao, status)
     VALUES ($1,$2,$3,$4,$5,$6,$7,'pendente')
     RETURNING *`,
    [data.empresaId, data.fornecedorId, data.descricao, data.categoria || null, data.valor, data.vencimento, data.observacao || null]
  );
...
async function atualizarContaPagar(id, body) {
  ...
  if (atual.status !== 'pendente') {
    return { errors: { status: 'Só é possível editar uma conta pendente — contas pagas ou canceladas não podem mais ser alteradas.' } };
  }
  ...
  const colMap = { fornecedorId: 'fornecedor_id', descricao: 'descricao', categoria: 'categoria', valor: 'valor', vencimento: 'vencimento', observacao: 'observacao' };
  const fields = [];
  const values = [];
  let i = 1;
...
// Usada SÓ pela conciliação bancária (lib/conciliacaoBancaria.js) — nunca
// chamada diretamente pela tela de Contas a Pagar. Mesma regra de sempre
// (nunca marca 2x, nunca "anda pra trás"): o UPDATE só afeta a linha se
// ainda não estiver paga/cancelada — condição no próprio WHERE (atômico,
// sem race entre ler e escrever), igual ao padrão já usado em
// recebimentosMl.js#marcarComoRecebidoPorConciliacao. Recebe um `client`
// de transação (a confirmação de conciliação grava o movimento do extrato
// E a conta a pagar juntos, ou nenhum dos dois).
async function marcarComoPagoPorConciliacao(client, id, dataPagamento) {
  const { rows } = await client.query(
    `UPDATE contas_pagar SET status='pago', data_pagamento=$1, updated_at=now()
     WHERE id=$2 AND status NOT IN ('pago','cancelado') RETURNING *`,
    [dataPagamento, id]
  );
