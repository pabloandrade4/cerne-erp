...
// 21/09/2026, pedido explícito do usuário (achou uma duplicidade de
// verdade em produção — ver docs/02-decisoes.md e docs/04-alteracoes.md):
// o código do documento (coluna "CR", ver ALIASES.documento acima) às
// vezes vem no arquivo COM o prefixo "CR" na frente do número (ex:
// "CR9801") e às vezes vem só o número (ex: "9801") — mesmo sendo o
// mesmíssimo título. Antes desta correção, a chave de duplicidade
// comparava o texto exatamente como veio (via slugHeader), então
// "CR9801" e "9801" eram tratados como documentos DIFERENTES e a mesma
// conta entrava duas vezes. Agora o prefixo "CR" (com ou sem espaço/
// hífen depois) é removido só para efeito de COMPARAÇÃO — o valor
// original do documento continua sendo guardado e mostrado exatamente
// como veio no arquivo, isso muda só a checagem de duplicidade.
function normalizarDocumentoParaChave(documento) {
  return slugHeader(documento || '').replace(/^cr[\s-]*/, '');
}
...
function chaveDuplicidade(dados, empresaId) {
  ...
  const documento = normalizarDocumentoParaChave(dados.documento);
