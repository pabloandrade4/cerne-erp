...
// Mapeia `dias` para a chave de período sincronizada em ads_metricas_anuncio
// (lib/ads.js#sincronizarContaAds sincroniza exatamente as chaves de
// lib/periodo.js — 'hoje'|'ontem'|'7d'|'30d'|'mes') — o Radar só usa 7 e 30.
// Sem correspondência direta, listarAds cai no padrão '30d' (nunca inventa,
// só usa a janela mais próxima já sincronizada).
function chaveDoPeriodoAds(dias) {
  if (dias === 7) return '7d';
...
async function listarAdsSeguro({ empresaId, dias, agora }) {
  try {
    ...
    return await listarAds({
      empresaId, contaId: null, periodoChave: chaveDoPeriodoAds(dias), desde, ate: agora, desdeStr, ateStr,
