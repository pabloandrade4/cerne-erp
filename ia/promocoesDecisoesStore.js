...
async function upsertDecisaoPromocao(linha, sugestao) {
  ...
  if (existente.rows.length) {
    ...
    await pool.query(
      `UPDATE ia_decisoes_promocoes SET
         ...
         snapshot_margem_normal_pct = $13,
         snapshot_estoque_atual = $14, snapshot_cobertura_dias_estoque = $15, snapshot_estoque_alto = $16,
         valor_sugerido_ia = $17, atualizado_em = now()
       WHERE id = $18`,
      [
        ...
        linha.margemReal, linha.margemRealPct, linha.margemNormalPct,
        linha.estoqueAtual, linha.coberturaDiasEstoque, !!linha.estoqueAlto,
  ...
  const { rows } = await pool.query(
    `INSERT INTO ia_decisoes_promocoes (
       ...
       snapshot_margem_real, snapshot_margem_real_pct, snapshot_margem_normal_pct,
       snapshot_estoque_atual, snapshot_cobertura_dias_estoque, snapshot_estoque_alto,
       valor_sugerido_ia
    ...
    [
      ...
      linha.margemReal, linha.margemRealPct, linha.margemNormalPct,
      linha.estoqueAtual, linha.coberturaDiasEstoque, !!linha.estoqueAlto,
      JSON.stringify(sugestao.valorSugeridoIa || {}),
    ]
  );
  return rows[0].id;
}
