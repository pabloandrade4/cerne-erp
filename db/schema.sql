...
-- Contas bancárias e importação de extratos — 31/08/2026.
-- O saldo final do extrato é a fotografia real do banco usada como base do
-- Fluxo de Caixa. As movimentações NÃO entram diretamente na fórmula do
-- fluxo para não duplicar contas_pagar/contas_receber; ficam disponíveis
-- para histórico e futura conciliação.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_extrato_importacoes_arquivo
  ON extrato_importacoes(conta_bancaria_id, arquivo_hash) WHERE arquivo_hash IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS extrato_movimentos (
  ...
  conciliacao_tipo  VARCHAR(30),
  conciliacao_id    BIGINT,
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE UNIQUE INDEX IF NOT EXISTS idx_extrato_movimentos_fingerprint
  ON extrato_movimentos(conta_bancaria_id, fingerprint);
CREATE INDEX IF NOT EXISTS idx_extrato_movimentos_empresa_data
  ON extrato_movimentos(empresa_id, data DESC);
