...
// CORREÇÃO (15/09/2026 — pedido explícito do usuário: "abri o mercado
// livre e vi que tem 2 pos vendas para responder, mas no sistema nao esta
// constando nada"): a versão anterior pegava só os 30 pedidos MAIS
// RECENTES (por data do pedido) — mas uma mensagem pós-venda pode chegar
// num pedido de semanas atrás (o comprador manda a mensagem quando quiser
// dentro da janela pós-venda, não necessariamente no dia da compra). Pegar
// por CONTAGEM de pedidos recentes podia deixar de fora justamente o
// pedido com a mensagem nova, se a loja tiver bastante volume. Agora a
// janela é por DATA (JANELA_MENSAGENS_DIAS), que cobre todo pedido dentro
// do período em que o Mercado Livre normalmente permite mensagem
// pós-venda — com um teto de segurança (LIMITE_PEDIDOS_PARA_MENSAGENS)
// só pra nunca disparar uma quantidade absurda de chamadas num ciclo só.
const JANELA_MENSAGENS_DIAS = Number(process.env.SAC_ML_JANELA_MENSAGENS_DIAS) || 60;
const LIMITE_PEDIDOS_PARA_MENSAGENS = Number(process.env.SAC_ML_LIMITE_PEDIDOS_MENSAGENS) || 150;
...
// (lib/mlSync.js), que já tem pack_id + dados do comprador. Olha todos os
// pedidos dos últimos JANELA_MENSAGENS_DIAS dias (não só uma contagem fixa
// dos mais recentes — ver comentário de JANELA_MENSAGENS_DIAS acima),
// até o teto de segurança LIMITE_PEDIDOS_PARA_MENSAGENS.
//
// Devolve também diagnóstico (pedidosVerificados, erros) — pedido implícito
// do usuário ao reportar "tem 2 pós-vendas no Mercado Livre mas no sistema
// não consta nada": sem esse diagnóstico, uma falha silenciosa (ex.: a API
// rejeitar o parâmetro, ou o formato da mensagem vir diferente do
// esperado) fica indistinguível de "não há mensagem nova" — nunca mais
// devolve só um "0" sem explicação.
async function sincronizarMensagensPosVenda({ empresaId, contaId, accessToken, sellerId }) {
    ...
    `SELECT pack_id, ml_order_id, comprador_nickname, comprador_id FROM (
       SELECT DISTINCT ON (pack_id) pack_id, ml_order_id, comprador_nickname, comprador_id, data_criacao
        ...
        WHERE conta_ml_id = $1 AND pack_id IS NOT NULL
          AND data_criacao >= now() - ($2 || ' days')::interval
        ORDER BY pack_id, data_criacao DESC
     ) mais_recente_por_pack
     ORDER BY data_criacao DESC
     LIMIT $3`,
    [contaId, JANELA_MENSAGENS_DIAS, LIMITE_PEDIDOS_PARA_MENSAGENS]
  );
  if (!pedidos.length) return { processadas: 0, novos: 0, idsNovos: [], pedidosVerificados: 0, erros: [] };
  ...
  for (const pedido of pedidos) {
    try {
      ...
      // nunca derruba os outros, mas o erro fica visível no resultado
      // (nunca só um console.error escondido do usuário)
      console.error(`[SAC Mercado Livre][mensagens] pack ${pedido.pack_id}: ${err.message}`);
      if (erros.length < 5) erros.push(`Pedido ${pedido.ml_order_id || pedido.pack_id}: ${err.message}`);
    }
  }
  return { processadas, novos, idsNovos, pedidosVerificados: pedidos.length, erros };
}
