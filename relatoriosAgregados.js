...
const { buscarPedidosDoPeriodo, resumirPeriodo, buscarItensDoPeriodoTodosCanais } = require('./relatorioVendas');
...
async function relatorioProdutos({ empresaId, contaId, desde, ate, sku }) {
  // 14/09/2026 (pedido explícito do usuário: "quero relatórios também igual
  // ao mercado livre") — inclui os itens da Shopee (rateados por pedido, já
  // que a Shopee não decompõe comissão por item — ver comentário em
  // buscarItensDoPeriodoTodosCanais). O filtro de loja abaixo (`contaId`)
  // continua recebendo só um id de conta do Mercado Livre por enquanto (o
  // seletor de loja desta tela ainda lista só contas do Mercado Livre) — os
  // itens da Shopee têm `contaMlId` null, então nunca "batem" um filtro de
  // loja do Mercado Livre por engano, e aparecem normalmente quando
  // nenhuma loja está selecionada.
  const { itens } = await buscarItensDoPeriodoTodosCanais({ empresaId, desde, ate });
...
async function relatorioProdutosPorCaixa({ empresaId, contaId, desde, ate }) {
  // Mesma extensão de relatorioProdutos acima (14/09/2026) — inclui itens
  // da Shopee, rateados por pedido.
  const { itens } = await buscarItensDoPeriodoTodosCanais({ empresaId, desde, ate });
...
async function relatorioMarketplaces({ empresaId, desde, ate }) {
  ...
  pedidos.forEach((p) => {
    // 14/09/2026: agrupa por `contaKey` (marketplace:contaId), NUNCA por
    // `contaMlId` sozinho — contaMlId vem null pra todo pedido Shopee (ver
    // comentário em lib/relatorioVendas.js#serializarPedido), então agrupar
    // por ele juntaria TODAS as lojas Shopee numa única linha "null". Cada
    // linha continua identificando a loja real (marketplace + contaMlId OU
    // contaShopeeId, o que valer pra ela).
    const chave = p.contaKey;
    if (!porConta.has(chave)) {
      porConta.set(chave, { marketplace: p.marketplace, contaMlId: p.contaMlId, contaShopeeId: p.contaShopeeId, loja: p.loja, pedidos: [] });
    }
    porConta.get(chave).pedidos.push(p);
  });

  const linhas = [...porConta.values()].map(({ marketplace, contaMlId, contaShopeeId, loja, pedidos: pedidosDaLoja }) => {
    const resumo = resumirPeriodo(pedidosDaLoja);
    return { marketplace, contaMlId, contaShopeeId, loja, resumo };
