...
//      permissões do usuário." — empresaId NUNCA é um parâmetro que o
//      modelo escolhe: fica preso no `contexto` (criado uma vez por
//      pergunta, a partir do que o front-end mandou do header) e cada
//      ferramenta só enxerga esse contexto, nunca um empresaId vindo do
//      texto da pergunta ou de uma decisão do modelo. Isso torna
//      estruturalmente impossível a IA responder com dado de uma empresa
//      diferente da selecionada. O período do cabeçalho segue essa mesma
//      regra para TODA ferramenta — com uma única exceção, pedida
//      explicitamente pelo usuário em 11/09/2026 (ver docs/04-alteracoes.md):
//      comparacao_periodo_anterior aceita duas datas explícitas quando o
//      USUÁRIO as escreve na própria pergunta (ex: "compare 01/08 a 10/08
//      com 01/09 a 10/09") — o modelo nunca inventa/estima essas datas por
//      conta própria, só repassa exatamente o que o usuário digitou; sem
//      datas explícitas, a ferramenta continua usando o período do
//      cabeçalho normalmente. (Sobre "permissões do usuário": desde a
...
const { calcularPeriodo, periodoParaDatasBRT, diaBRT, inicioDoDiaBRTDeString } = require('../periodo');
...
// CORREÇÃO (01/09/2026, ativação do Ads/Central de Alertas — Etapa 8 pedida
// pelo usuário, ver docs/04-alteracoes.md): `gerarAlertas` acima (Visão
// Geral > Alertas & IA) NUNCA teve regra nenhuma de Ads (confirmado por
// inspeção do arquivo) — só o Radar da IA (lib/ia/radar.js, persistido em
// radar_alertas, mesma fonte agora usada pela Central de Alertas) detecta
// "anúncio gastando sem vender"/"anúncio consumindo margem"/"anúncio com
// prejuízo depois do Ads". Sem isto, a ferramenta `alertas_operacionais`
// (usada por perguntas como "quais problemas precisam da minha atenção")
// nunca conseguiria mencionar um problema de Ads — mesmo a IA tendo acesso
// à ferramenta `ads_desempenho` separadamente.
const { listarAlertasCentral } = require('./radar');
...
// Despesas por categoria (31/08/2026) — mesma fonte já usada pela DRE
// ampliada (lib/despesasFinanceiras.js), nenhum cálculo novo aqui.
const { listarDespesasDetalhadas } = require('../despesasFinanceiras');
const { listarCategoriasFlat } = require('../categoriasFinanceiras');
...
const PERIODO_CUSTOM_MAX_DIAS = 366; // proteção contra uma janela absurda (ex: 20 anos) numa pergunta

// Converte uma data 'AAAA-MM-DD' (a única forma aceita — nunca "01/08/2026"
// nem texto livre) no instante 00:00:00 BRT correspondente, ou null se o
// texto não bater exatamente nesse formato ou não for uma data real (ex:
// "2026-02-30"). Usado só por comparacao_periodo_anterior, no modo em que o
// USUÁRIO escreveu datas explícitas na pergunta — nunca para o modelo
// "adivinhar" uma data.
function parseDataYYYYMMDD(str) {
  if (typeof str !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(str.trim())) return null;
  const texto = str.trim();
  const instante = inicioDoDiaBRTDeString(texto);
  if (Number.isNaN(instante.getTime())) return null;
  // Date "rola" um dia inválido (ex: 30 de fevereiro vira 2 de março) em vez
  // de falhar — o round-trip por diaBRT pega esse caso.
  if (diaBRT(instante) !== texto) return null;
  return instante;
}
...
async function handleAlertasOperacionais(_input, ctx) {
  const [pedidos, itens, conexoes, contasAPagar, contasAReceber, radarCentral] = await Promise.all([
    ctx.pedidos(),
    ctx.itens(),
    conexoesEEmpresas(ctx.empresaId),
    resumoContasPagar({ empresaId: ctx.empresaId, desde: ctx.desdeStr, ate: ctx.ateStr }),
    resumoContasReceber({ empresaId: ctx.empresaId, desde: ctx.desdeStr, ate: ctx.ateStr }),
    // Central de Alertas (Radar da IA) — única fonte com regras de Ads
    // (gerarAlertas acima nunca teve; ver comentário no require acima).
    // .catch: nunca deixa um problema no Radar quebrar esta ferramenta —
    // a IA continua respondendo com os alertas de Visão Geral mesmo assim.
    listarAlertasCentral(ctx.empresaId).catch(() => ({ alertas: [] })),
  ]);
  const alertas = await gerarAlertas({ empresaId: ctx.empresaId, pedidos, itens, fluxoCaixa: { contasAPagar, contasAReceber }, conexoes });
  // Só os "ainda abertos de verdade" (Novo/Visualizado) — nunca os que o
  // usuário já marcou como Resolvido/Ignorado na Central de Alertas.
  const alertasRadar = (radarCentral.alertas || []).filter((a) => a.statusCentral === 'novo' || a.statusCentral === 'visualizado');

  const combinados = [
    ...alertas.map((a) => ({ tipo: a.tipo, severidade: a.severidade, titulo: a.titulo, descricao: a.descricao, telaRelacionada: a.pagina, origem: 'visao_geral' })),
    ...alertasRadar.map((a) => ({ tipo: a.categoria, severidade: a.severidade, titulo: a.titulo, descricao: a.descricao, telaRelacionada: a.pagina, origem: 'central_de_alertas' })),
  ];

  return {
    periodo: ctx.periodoCalc.label,
    quantidadeDeAlertas: combinados.length,
    alertas: combinados,
    observacao: combinados.length
      ? 'Junta os alertas de Visão Geral > Alertas & IA (origem "visao_geral") com os da Central de Alertas/Radar da IA (origem "central_de_alertas", inclui Ads) — nunca inclui um alerta que o usuário já marcou como Resolvido ou Ignorado na Central de Alertas.'
      : 'Nenhum alerta ativo no momento, nem em Visão Geral > Alertas & IA nem na Central de Alertas (Ads incluído).',
  };
}
...
async function handleFluxoDeCaixa(_input, ctx) {
  ...
  return {
    ...
    saldoBancarioAtual: fluxo.saldoBancarioAtual.valor !== null
      ? { valor: fluxo.saldoBancarioAtual.valor, valorFormatado: formatMoneyOuNull(fluxo.saldoBancarioAtual.valor), dataReferencia: fluxo.saldoBancarioAtual.dataReferencia, disponivel: true }
      : { valor: null, disponivel: false, motivo: fluxo.saldoBancarioAtual.motivo },
    saldoProjetado: { valor: null, disponivel: false, motivo: fluxo.saldoProjetado.motivo },
    observacao: 'saldoBancarioAtual é o saldo real das contas bancárias (soma do saldo final do último extrato importado de cada conta — mesmo número da tela Contas Bancárias/Fluxo de Caixa); fica indisponível só se nenhuma conta bancária tiver saldo importado ainda. "saldoProjetado" (fim do período à frente, somando contas a pagar/receber futuras dia a dia) não é calculado por esta ferramenta — para essa projeção detalhada, oriente o usuário a abrir a tela Fluxo de Caixa. Sempre separe o que já ACONTECEU ("realizado") do que é só uma EXPECTATIVA ("previsto"/"projetado") — nunca apresente um valor previsto como dinheiro já disponível.',
  };
}
...
async function handleDreCompleta(_input, ctx) {
  ...
  return {
    ...
    linhas: {
      ...
      despesasDoPeriodo: linhaResposta(l.despesasPeriodo),
      resultadoFinal: linhaResposta(l.resultadoFinal),
    },
    despesasPorCategoria: dre.despesas.porCategoria.slice(0, 15).map((g) => ({
      categoria: g.categoria,
      total: g.total,
      totalFormatado: formatMoneyOuNull(g.total),
      quantidadeLancamentos: g.quantidade,
      subcategorias: (g.subcategorias || []).map((s) => ({ subcategoria: s.subcategoria, total: s.total, totalFormatado: formatMoneyOuNull(s.total), quantidadeLancamentos: s.quantidade })),
    })),
    cardsDespesas: {
      totalDespesas: dre.despesas.cards.totalDespesas,
      totalDespesasFormatado: formatMoneyOuNull(dre.despesas.cards.totalDespesas),
      maiorCategoria: dre.despesas.cards.maiorCategoria,
      segundaMaiorCategoria: dre.despesas.cards.segundaMaiorCategoria,
      quantidadeDespesas: dre.despesas.cards.quantidadeDespesas,
      mediaPorDia: dre.despesas.cards.mediaPorDia,
    },
    observacao: 'Cada linha em R$ e em % sobre a Receita Bruta. Sem nenhum pedido no período inteiro, todas as linhas de receita vêm "Sem dados" (disponivel:false, pedidosSemEssaInformacao:0) — diferente de "Pendente" (disponivel:false com pedidosSemEssaInformacao>0), que significa que havia pedido mas faltou alguma informação (ex: custo de SKU). A Margem de Contribuição é sempre a mesma já mostrada em Pedidos/Visão Geral/Financeiro — nunca recalculada por soma das outras linhas. "despesasDoPeriodo" = contas a pagar pagas no período + saídas do extrato bancário que nunca viraram conta a pagar, sem contar a mesma saída duas vezes e sem incluir transferência entre contas da própria empresa — "despesasPorCategoria"/"cardsDespesas" são o MESMO total quebrado por categoria (útil para "para onde está indo o dinheiro"); para os lançamentos individuais de uma categoria específica, use a ferramenta despesas_por_categoria.',
  };
}

// Resolve um nome de categoria digitado pelo usuário pro cadastro real
// (lib/categoriasFinanceiras.js) — o modelo nunca sabe o ID de uma
// categoria, só o nome que o usuário disse. Casamento exato primeiro,
// depois por substring (case-insensitive) — nunca inventa uma categoria que
// não existe no cadastro desta empresa.
async function resolverCategoriaPorNome(empresaId, nomeDigitado) {
  if (!nomeDigitado) return null;
  const categorias = await listarCategoriasFlat({ empresaId, incluirInativas: true });
  const alvo = String(nomeDigitado).trim().toLowerCase();
  if (!alvo) return null;
  return categorias.find((c) => c.nome.toLowerCase() === alvo)
    || categorias.find((c) => c.nome.toLowerCase().includes(alvo))
    || null;
}

// Despesas por categoria / busca livre — casca fina sobre
// lib/despesasFinanceiras.js#listarDespesasDetalhadas, a MESMA lista que
// alimenta o bloco "Despesas por categoria" e o gráfico "Para onde está
// indo o dinheiro?" da tela DRE (nunca uma segunda consulta com filtro
// diferente). Cobre perguntas de despesa que dre_completa não detalha a
// nível de lançamento: "quanto gastei em [categoria]", "quanto gastei com
// [fornecedor]", "busca a despesa do CR/documento X", "tem alguma despesa
// sem categoria".
async function handleDespesasPorCategoria(input, ctx) {
  const nomeCategoriaBuscada = input && input.categoria ? String(input.categoria).trim() : null;
  const categoriaEncontrada = nomeCategoriaBuscada ? await resolverCategoriaPorNome(ctx.empresaId, nomeCategoriaBuscada) : null;

  if (nomeCategoriaBuscada && !categoriaEncontrada) {
    return {
      periodo: ctx.periodoCalc.label,
      categoriaEncontrada: false,
      categoriaBuscada: nomeCategoriaBuscada,
      despesas: [],
      quantidadeEncontrada: 0,
      observacao: 'Nenhuma categoria com esse nome foi encontrada no cadastro desta empresa (Financeiro > Contas a Pagar > "gerenciar categorias"). Confira o nome exato com o usuário, ou use a ferramenta dre_completa (campo despesasPorCategoria) para ver a lista de categorias que realmente têm despesa no período.',
    };
  }
  ...
  const despesas = await listarDespesasDetalhadas({
    ...
    search: input && input.busca ? input.busca : null,
  });
  const semCategoria = input && input.semCategoria === true ? despesas.filter((d) => !d.categoria) : despesas;
  const limite = Math.min(Math.max(parseInt(input && input.limite, 10) || 20, 1), 60);
  const total = round2(semCategoria.reduce((s, d) => s + d.valor, 0));

  return {
    periodo: ctx.periodoCalc.label,
    categoriaFiltrada: categoriaEncontrada ? categoriaEncontrada.nome : null,
    quantidadeEncontrada: semCategoria.length,
    total,
    totalFormatado: formatMoneyOuNull(total),
    despesas: semCategoria.slice(0, limite).map((d) => ({
      data: d.data,
      descricao: d.descricao,
      categoria: d.categoria,
      subcategoria: d.subcategoria,
      fornecedor: d.fornecedor,
      documentoOuCr: d.documento,
      contaBancaria: d.contaBancaria,
      origem: d.origem,
      valor: d.valor,
      valorFormatado: formatMoneyOuNull(d.valor),
    })),
    listaTruncada: semCategoria.length > limite,
    observacao: 'Une, sem duplicar, contas a pagar pagas no período selecionado e saídas do extrato bancário que nunca viraram conta a pagar (uma conta a pagar já conciliada com o extrato conta só uma vez) — nunca inclui transferência entre contas da própria empresa. "busca" filtra por descrição, fornecedor, CR/documento ou nome da categoria. Para o total de TODAS as categorias de uma vez (sem escolher uma por vez), use dre_completa.',
  };
}
...
// Comparação entre dois períodos. Dois modos:
//
//   1) PADRÃO (sem nenhuma das 4 datas no input): compara o período
//      selecionado no cabeçalho com o período anterior EQUIVALENTE (mesma
//      duração em dias, terminando exatamente onde o período selecionado
//      começa) — calculado deterministicamente, nunca escolhido pelo
//      modelo.
//   2) DATAS EXPLÍCITAS (as 4 informadas): usado quando o próprio USUÁRIO
//      escreve duas datas específicas na pergunta (ex: "compare 01/08 a
//      10/08 com 01/09 a 10/09") — pedido explícito do usuário em
//      11/09/2026 (ver docs/04-alteracoes.md). O modelo só preenche esses
//      campos com o que o usuário efetivamente digitou, nunca com uma data
//      inventada/estimada — a regra "nunca inventar" (topo deste arquivo)
//      vale aqui também: se faltar alguma data ou o formato estiver
//      errado, devolve um erro claro em vez de adivinhar.
async function handleComparacaoPeriodoAnterior(input, ctx) {
  const { periodoADesde, periodoAAte, periodoBDesde, periodoBAte } = input || {};
  const algumaDataInformada = periodoADesde || periodoAAte || periodoBDesde || periodoBAte;

  let atualDesde, atualAte, anteriorDesde, anteriorAte, labelAtual, labelAnterior, reaproveitarCtxPedidos;

  if (algumaDataInformada) {
    if (!periodoADesde || !periodoAAte || !periodoBDesde || !periodoBAte) {
      return { erro: 'Para comparar duas datas específicas, informe as 4: periodoADesde, periodoAAte, periodoBDesde e periodoBAte (todas no formato AAAA-MM-DD, ex: "2026-08-01"). Se o usuário não disse alguma dessas datas, pergunte a ele em vez de adivinhar.' };
    }
    const dAtualDesde = parseDataYYYYMMDD(periodoADesde);
    const dAtualAteBase = parseDataYYYYMMDD(periodoAAte);
    const dAnteriorDesde = parseDataYYYYMMDD(periodoBDesde);
    const dAnteriorAteBase = parseDataYYYYMMDD(periodoBAte);
    if (!dAtualDesde || !dAtualAteBase || !dAnteriorDesde || !dAnteriorAteBase) {
      return { erro: 'Uma ou mais datas não são válidas. Use sempre o formato AAAA-MM-DD (ano-mês-dia, ex: "2026-08-01"), com um dia/mês/ano que realmente existam.' };
    }
    if (dAtualAteBase.getTime() < dAtualDesde.getTime() || dAnteriorAteBase.getTime() < dAnteriorDesde.getTime()) {
      return { erro: 'A data final de cada período precisa ser igual ou depois da data inicial dele.' };
    }
    atualDesde = dAtualDesde;
    atualAte = new Date(dAtualAteBase.getTime() + UM_DIA_MS); // "ate" é exclusivo — soma 1 dia pra incluir o dia final inteiro
    anteriorDesde = dAnteriorDesde;
    anteriorAte = new Date(dAnteriorAteBase.getTime() + UM_DIA_MS);
    if (atualAte.getTime() - atualDesde.getTime() > PERIODO_CUSTOM_MAX_DIAS * UM_DIA_MS
      || anteriorAte.getTime() - anteriorDesde.getTime() > PERIODO_CUSTOM_MAX_DIAS * UM_DIA_MS) {
      return { erro: `Cada período pode ter no máximo ${PERIODO_CUSTOM_MAX_DIAS} dias.` };
    }
    labelAtual = `${periodoADesde} a ${periodoAAte}`;
    labelAnterior = `${periodoBDesde} a ${periodoBAte}`;
    reaproveitarCtxPedidos = false;
  } else {
    const duracaoMs = ctx.periodoCalc.ate.getTime() - ctx.periodoCalc.desde.getTime();
    atualDesde = ctx.periodoCalc.desde;
    atualAte = ctx.periodoCalc.ate;
    anteriorAte = ctx.periodoCalc.desde;
    anteriorDesde = new Date(anteriorAte.getTime() - duracaoMs);
    labelAtual = ctx.periodoCalc.label;
    labelAnterior = 'Período anterior equivalente (mesma duração)';
    reaproveitarCtxPedidos = true;
  }

  const [pedidosAtual, { pedidos: pedidosAnterior }] = await Promise.all([
    reaproveitarCtxPedidos ? ctx.pedidos() : buscarPedidosDoPeriodo({ empresaId: ctx.empresaId, desde: atualDesde, ate: atualAte }).then((r) => r.pedidos),
  ...
  return {
    periodoAtual: { label: labelAtual, desde: diaBRT(atualDesde), ate: diaBRT(new Date(atualAte.getTime() - 1)) },
    periodoAnterior: { label: labelAnterior, desde: diaBRT(anteriorDesde), ate: diaBRT(new Date(anteriorAte.getTime() - 1)) },
    faturamento: {
    ...
    observacao: algumaDataInformada
      ? 'Comparação entre as duas datas exatas informadas na pergunta (nunca escolhidas pelo modelo).'
      : 'O período anterior é sempre a mesma duração do período selecionado no cabeçalho, terminando exatamente onde ele começa (ex: se o cabeçalho está em "Últimos 7 dias", compara com os 7 dias imediatamente anteriores). Para comparar duas datas específicas em vez do período do cabeçalho, informe periodoADesde/periodoAAte/periodoBDesde/periodoBAte.',
  };
}
...
const FERRAMENTAS = [
  ...
  {
    name: 'fluxo_de_caixa',
    description: 'Componentes reais do fluxo de caixa da empresa selecionada: saldo bancário atual (soma do saldo do último extrato importado de cada conta, quando houver), o que já foi recebido/pago no período (realizado), contas a receber/pagar em aberto, vencendo nos próximos 7 dias e atrasadas/vencidas (previsto), e recebimentos esperados do Mercado Livre. NÃO inclui a projeção dia-a-dia até o fim de um período futuro (isso só existe na tela Fluxo de Caixa). Use para "como está meu fluxo de caixa", "quanto tenho no banco", "o que vence esta semana" ou perguntas hipotéticas de caixa (explique com os números reais, nunca com um saldo final inventado).',
    input_schema: { type: 'object', properties: {}, additionalProperties: false },
    handler: handleFluxoDeCaixa,
  },
  {
    name: 'dre_completa',
    description: 'A DRE (Demonstrativo de Resultado) completa da empresa e período selecionados, linha por linha, em R$ e %: Receita Bruta, Cancelamentos, Descontos, Receita Líquida, Custo dos Produtos, Taxas e Comissões, Frete do Vendedor, Impostos, Margem de Contribuição, Despesas do período (contas a pagar + extrato, já quebradas por categoria em despesasPorCategoria/cardsDespesas), Resultado Final. Use quando o usuário pedir a DRE em si, o resultado final, ou "para onde está indo o dinheiro" de forma geral (ex: "mostre minha DRE deste mês", "qual minha maior categoria de despesa") — para só o resultado final resumido, prefira resultado_periodo; para os LANÇAMENTOS individuais de uma categoria/fornecedor/CR específico, use despesas_por_categoria.',
    input_schema: { type: 'object', properties: {}, additionalProperties: false },
    handler: handleDreCompleta,
  },
  {
    name: 'despesas_por_categoria',
    description: 'Lista os LANÇAMENTOS individuais de despesa (contas a pagar pagas + saídas do extrato bancário nunca conciliadas) da empresa e período selecionados, com filtro opcional por categoria (nome) e/ou busca livre por descrição/fornecedor/CR/documento — mesma fonte da tela DRE > Despesas por categoria. Use para "quanto gastei em [categoria]", "quanto gastei com [fornecedor]", "mostre a despesa do CR/documento X", "tem despesa sem categoria". Para o total já quebrado por TODAS as categorias de uma vez, prefira dre_completa.',
    input_schema: {
      type: 'object',
      properties: {
        categoria: { type: 'string', description: 'Nome de uma categoria ou subcategoria financeira (ex: "Alimentação", "Frete"). Casamento por nome, nunca por ID — se não existir uma categoria com esse nome no cadastro desta empresa, a ferramenta avisa em vez de inventar.' },
        busca: { type: 'string', description: 'Texto livre para buscar em descrição, fornecedor, CR/documento ou nome da categoria.' },
        semCategoria: { type: 'boolean', description: 'true para listar só despesas SEM nenhuma categoria atribuída (útil para "quais despesas ainda não foram categorizadas").' },
        limite: { type: 'integer', minimum: 1, maximum: 60, description: 'Quantos lançamentos retornar (padrão 20). O total em R$ sempre soma TODOS os encontrados, mesmo além do limite da lista.' },
      },
      additionalProperties: false,
    },
    handler: handleDespesasPorCategoria,
  },
  {
    name: 'compras_resumo',
  ...
  },
  {
    ...
    description: 'Compara faturamento, margem de contribuição e quantidade de pedidos entre dois períodos. Sem nenhum parâmetro: compara o período selecionado no cabeçalho com o período imediatamente anterior de mesma duração (ex: "Últimos 7 dias" compara com os 7 dias antes desses) — use para "o que piorou/melhorou", "como estou comparado ao período anterior" ou "comparação mês a mês". Com periodoADesde/periodoAAte/periodoBDesde/periodoBAte: compara duas datas ESPECÍFICAS — use SOMENTE quando o usuário escrever as duas datas explicitamente na pergunta (ex: "compare 01/08 a 10/08 com 01/09 a 10/09"); nunca invente essas datas por conta própria, e informe as 4 juntas ou nenhuma.',
    input_schema: {
      type: 'object',
      properties: {
        periodoADesde: { type: 'string', description: 'Data de início do primeiro período, no formato AAAA-MM-DD (ex: "2026-08-01"). Use só quando o usuário escreveu essa data explicitamente na pergunta. Se usar este campo, os outros 3 (periodoAAte, periodoBDesde, periodoBAte) também são obrigatórios.' },
        periodoAAte: { type: 'string', description: 'Data final (inclusive) do primeiro período, no formato AAAA-MM-DD.' },
        periodoBDesde: { type: 'string', description: 'Data de início do segundo período (o que será comparado com o primeiro), no formato AAAA-MM-DD.' },
        periodoBAte: { type: 'string', description: 'Data final (inclusive) do segundo período, no formato AAAA-MM-DD.' },
      },
      additionalProperties: false,
    },
