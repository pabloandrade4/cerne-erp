...
async function sincronizarEstoqueConta(contaId, { maxPaginas = MAX_PAGINAS_PADRAO } = {}) {
  ...
  // docs/04-alteracoes.md): antes, esta função fazia `pool.connect()` e
  // segurava UMA conexão dedicada do pool durante TODA a sincronização
  // (podendo ser várias páginas × várias chamadas seriais à API do
  // Mercado Livre, sem limite de tempo total) — cada conta ativa faz isso
  // ao mesmo tempo (Promise.allSettled em lib/syncScheduler.js). Isso
  // consumia conexões do pool por muito tempo sem necessidade real (os
  // upserts abaixo não precisam de transação — cada um já é atômico via
  // ON CONFLICT), e foi uma causa confirmada, junto com a falta de
  // timeout no pool (ver db/pool.js), do esgotamento de conexões visto em
  // produção (`EMAXCONNSESSION`) e do ciclo de sincronização travando por
  // 34+ horas. Agora usa `pool.query()` direto (pega/devolve uma conexão
  // por operação, como o resto do arquivo já fazia em processarItem antes
  // desta correção), nunca prendendo uma conexão além do necessário.
  try {
    while (paginas < maxPaginas) {
      ...
      for (const item of itensDaPagina) {
        await processarItem(pool, item, conta, accessToken);
      }
    ...
    // nada a liberar — pool.query() (chamado dentro de processarItem/
    // upsertLinha) já devolve a conexão sozinho a cada chamada.
  }
